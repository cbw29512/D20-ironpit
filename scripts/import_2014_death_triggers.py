...
def parse_death_trigger(paragraph: str) -> dict | None:
    ...
    base = {
        ...
        "dc": dc,
        "range_ft": area.get("radius_ft", area.get("length_ft", 0)),
        "area": area,
    ...
    if damage is not None:
        ...
        half = bool(re.search(r"half as much damage on a successful", text, re.I))
        zero = bool(re.search(r"must succeed[^.]+or take", text, re.I))
        if not half and not zero:
            return None
        return {
            **base,
            "damage_dice_count": count,
            "damage_dice_size": size,
            "damage_bonus": bonus,
            "damage_type": damage_type,
            "success_damage": "half" if half else "none",
        }
