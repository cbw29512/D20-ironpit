...
def main() -> int:
    ...
    if digest != manifest["sha256"]:
        raise RuntimeError(f"2014 source drift: expected {manifest['sha256']}, got {digest}.")
    with tempfile.NamedTemporaryFile(suffix=".json", delete=False) as handle:
        handle.write(payload); source = Path(handle.name)
    try:
        commands = [
        ...
        if len(catalog) != manifest["monster_count"]:
            raise RuntimeError(f"2014 catalog count mismatch: expected {manifest['monster_count']}, got {len(catalog)}.")
