...
_PRONE_SAVE = re.compile(r"(?:If (?:the )?target is (?:(?:a )?(Tiny|Small|Medium|Large|Huge) or smaller|a creature),?\s*)?(?:the target|it) must succeed on a DC (\d+) (Strength|Dexterity|Constitution|Intelligence|Wisdom|Charisma) saving throw or be knocked prone\.?'?", re.I)
