...
_HIDDEN_RIDER = re.compile(
    ...
    r"|\b(?:Strength|Dexterity|Constitution|Intelligence|Wisdom|Charisma) score (?:decreases|is reduced)\b"
...
def _source_blockers(row: dict[str, object], monster_names: set[str]) -> list[str]:
    ...
    try:
        ...
        if any(not is_supported_monster_bonus_action(name) for name in bonus):
