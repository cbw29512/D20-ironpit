...
def _control(effect: Any) -> dict[str, Any] | None:
    ...
    if effect.condition_id:
        ...
        if effect.repeat_save_ability:
            row["repeatSaveAbility"] = effect.repeat_save_ability; row["repeatSaveDc"] = effect.repeat_save_dc
    ...
    try:
        ...
        if attack.on_hit_damage:
            row["onHitDamage"] = [{"source": part.source, "diceCount": part.dice_count, "diceSize": part.dice_size,
                                    "damageBonus": part.damage_bonus, "damageType": part.damage_type.value} for part in attack.on_hit_damage]
        ...
        if attack.conditional_damage:
            ...
            else: row["conditionalDamage"] = {"trigger": conditional.trigger, "mode": conditional.mode, "diceCount": conditional.dice_count,
                                                   "diceSize": conditional.dice_size, "damageBonus": conditional.damage_bonus, "damageType": conditional.damage_type.value}
        ...
        if CombatTrait.CHARGE.value in traits:
            ...
            if profile:
                ...
                if profile.replacement_damage is not None:
                    replacement = profile.replacement_damage; charge["replacementDamage"] = {"diceCount": replacement.dice_count, "diceSize": replacement.dice_size,
...
def _save(action: Any) -> dict[str, Any]:
    row: dict[str, Any] = {"id": action.id, "name": action.name, "saveAbility": action.save_ability, "dc": action.dc,
                           "range": action.range_ft, "damageDiceCount": action.damage_dice_count, "damageDiceSize": action.damage_dice_size,
                           "damageBonus": action.damage_bonus, "damageType": action.damage_type, "successDamage": action.success_damage, "animation": action.animation}
    ...
    if action.magical: row["magical"] = True
    if action.resource_id: row["resourceId"] = action.resource_id; row["resourceCost"] = action.resource_cost
    if action.area_slots != 1: row["areaSlots"] = action.area_slots
    if action.priority: row["priority"] = action.priority
    return row


def _spell(action: Any) -> dict[str, Any]:
    row = {"id": action.id, "name": action.name, "level": action.level, "actionCost": action.action_cost, "range": action.range_ft,
           "saveAbility": action.save_ability, "dc": action.dc, "damageDiceCount": action.damage_dice_count,
           "damageDiceSize": action.damage_dice_size, "damageBonus": action.damage_bonus, "damageType": action.damage_type,
           "successDamage": action.success_damage, "upcastDicePerLevel": action.upcast_dice_per_level, "concentration": action.concentration, "animation": action.animation}
...
def _spell_attack(action: Any) -> dict[str, Any]:
    row = {"id": action.id, "name": action.name, "level": action.level, "actionCost": action.action_cost, "range": action.range_ft,
           "attackBonus": action.attack_bonus, "damageDiceCount": action.damage_dice_count, "damageDiceSize": action.damage_dice_size,
           "damageBonus": action.damage_bonus, "damageType": action.damage_type, "onHitModifierEffects": [_modifier_effect(effect) for effect in action.on_hit_modifier_effects],
...
def _healing(action: Any) -> dict[str, Any]:
    return {"id": action.id, "name": action.name, "actionCost": action.action_cost, "range": action.range_ft, "targetMode": action.target_mode,
            "diceCount": action.dice_count, "diceSize": action.dice_size, "healingBonus": action.healing_bonus,
            "resourceId": action.resource_id, "resourceCost": action.resource_cost, "animation": action.animation}


def defense_row(action: Any) -> dict[str, Any]:
    row = {"id": action.id, "name": action.name, "level": action.level, "actionCost": action.action_cost, "range": action.range_ft,
           "durationMinutes": action.duration_minutes, "targetPolicy": action.target_policy, "targetCount": action.target_count,
           "temporaryHp": action.temporary_hp, "temporaryHpPerSlotAbove": action.temporary_hp_per_slot_above,
           "damageResistances": list(action.damage_resistances), "modifierEffects": [_modifier_effect(effect) for effect in action.modifier_effects],
...
def _progression_features(template: CombatantTemplate) -> dict[str, Any]:
    features = template.progression_features; row: dict[str, Any] = {}
...
def template_row(template: CombatantTemplate) -> dict[str, Any]:
    try:
        ...
        row: dict[str, Any] = {"id": template.id, "name": template.name, "archetype": template.archetype, "level": template.level,
            "challenge_rating": template.challenge_rating, "kind": template.kind, "size": template.size.value, "armor_class": template.armor_class,
            "max_hp": template.max_hp, "speed_ft": template.speed_ft, "movement_modes": template.movement_modes.model_dump(), "initiative_bonus": template.initiative_bonus,
            "saving_throw_bonuses": template.saving_throw_bonuses, "skill_bonuses": template.skill_bonuses, "attacks": [attack_row(item, traits) for item in attacks],
            ...
            "damage_resistances": [item.value for item in template.damage_resistances], "damage_vulnerabilities": [item.value for item in template.damage_vulnerabilities],
            "damage_immunities": [item.value for item in template.damage_immunities], "condition_immunities": list(template.condition_immunities),
            "visual": {"armor": template.visual.armor, "main_hand": template.visual.main_hand, "off_hand": template.visual.off_hand, "body_style": template.visual.body_style},
        ...
        if recharge: row["resource_recharge"] = recharge
        if template.attack_roll_advantage_triggers: row["attack_roll_advantage_triggers"] = list(template.attack_roll_advantage_triggers)
        if template.saving_throw_advantage_triggers: row["saving_throw_advantage_triggers"] = list(template.saving_throw_advantage_triggers)
        if template.kind == "monster":
            row.update(source_trait_names=list(template.source_trait_names), source_reaction_names=list(template.source_reaction_names),
                       source_bonus_action_names=list(template.source_bonus_action_names), source_limited_use_names=list(template.source_limited_use_names),
                       source_legendary_action_names=list(template.source_legendary_action_names), source_spellcasting_fingerprint=template.source_spellcasting_fingerprint)
        ...
        if template.attack_action: row["attack_action"] = {"id": template.attack_action.id, "name": template.attack_action.name,
            "slots": [{"attackIds": slot.attack_ids, "saveActionIds": slot.save_action_ids} for slot in template.attack_action.slots]}
        return row
    except Exception:
        logger.exception("Failed to serialize combatant template %s.", template.id); raise
