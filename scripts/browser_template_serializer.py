...
def template_row(template: CombatantTemplate) -> dict[str, Any]:
    try:
        ...
        if recharge_resources:
            row["resource_definitions"] = [
                {"id": item.id, "name": item.name, "maxUses": item.max_uses,
                 "recharge": {"trigger": item.recharge.trigger, "dieSize": item.recharge.die_size, "minimumRoll": item.recharge.minimum_roll}}
                for item in recharge_resources
            ]
        if template.kind == "monster":
            row["source_trait_names"] = list(template.source_trait_names); row["source_reaction_names"] = list(template.source_reaction_names); row["source_bonus_action_names"] = list(template.source_bonus_action_names); row["source_limited_use_names"] = list(template.source_limited_use_names); row["source_legendary_action_names"] = list(template.source_legendary_action_names); row["source_spellcasting_fingerprint"] = template.source_spellcasting_fingerprint
        ...
        if template.attack_action: row["attack_action"] = {"id": template.attack_action.id, "name": template.attack_action.name, "slots": [{"attackIds": slot.attack_ids, "saveActionIds": slot.save_action_ids} for slot in template.attack_action.slots]}
        return row
    except Exception:
