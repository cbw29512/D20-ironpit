    ...
    try:
        ...
        for key in sorted(set(by_2014) & set(by_2024)):
            ...
            statuses.append(PairedMonsterStatus(
                key=key,
                name_2014=monster_2014.name,
                name_2024=card_2024.name,
                ready_2014=runtime_ready_2014,
                ready_2024=card_2024.coverage_status is CoverageStatus.RAW_READY,
                blockers_2014=blockers_2014,
                blockers_2024=tuple(card_2024.blockers),
            ))
...
def main() -> None:
    try:
        ...
        ready_2014 = sum(item.ready_2014 for item in statuses) + len(only_2014) * 0
        total_ready_2014 = len(build_basic_2014_monsters())
        total_ready_2024 = sum(
            card.coverage_status is CoverageStatus.RAW_READY for card in build_monster_catalog()
        )
        ...
        for item in sorted(catchup_2014, key=lambda row: (len(row.blockers_2014), row.blockers_2014, row.name_2014))[:_REPORT_LIMIT]:
            print(f"PAIRED_CATCHUP_2014\t{item.name_2014}\t{_blocker_text(item.blockers_2014)}")
