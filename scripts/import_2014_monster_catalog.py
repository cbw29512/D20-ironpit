...
_ROLLED_DAMAGE = re.compile(
    r"Hit:\s*(\d+)\s*\((\d+)d(\d+)(?:\s*([+\-−])\s*(\d+))?\)\s*([A-Za-z]+) damage", re.I
)
...
def _primary_damage(text: str) -> tuple[re.Match[str], dict] | None:
    ...
    if rolled:
        average, count, size, sign, bonus, damage_type = rolled.groups(); damage_type = damage_type.lower()
        if damage_type not in DAMAGE_TYPES: return None
        modifier = int(bonus or 0) * (-1 if sign in {"-", "−"} else 1)
        return rolled, {"average": int(average), "dice_count": int(count), "dice_size": int(size), "bonus": modifier, "type": damage_type}
...
def _attack(paragraph: str) -> dict | None:
    text = _plain(paragraph); name_match = re.search(r"<strong>(.*?)</strong>", paragraph, re.I | re.S); hit = re.search(r"([+-]\d+) to hit", text, re.I)
    ...
    result = {"id": _slug(name), "name": name, "kind": kind, "attack_bonus": int(hit.group(1)), "damage": damage,
              "on_hit_damage": extras, "on_hit_save_effect": save_effect, "control_effect": control,
              "forbid_target_grappled_by_self": forbid_grappled, "source_complete": not dual_mode and not residual,
...
def _attacks(paragraph: str) -> list[dict]:
    ...
    damage_match, _ = primary; extras, residual = parse_secondary_damage(text[damage_match.end():].strip(" .")); save_effect, residual = parse_on_hit_save_condition(residual)
    control, forbid_grappled, residual = parse_on_hit_control(residual)
    if residual: return [attack]
    base_id = attack["id"]
    shared = {**attack, "on_hit_damage": extras, "on_hit_save_effect": save_effect, "control_effect": control,
              "forbid_target_grappled_by_self": forbid_grappled, "source_complete": True, "unsupported_text": None}
...
def _record(source: dict) -> dict:
    ...
    return {
        "id": _slug(source["name"]), "name": source["name"], "ruleset": "2014", "size": size.title(), "creature_type": creature_type.strip().split(" ")[0], "alignment": alignment.strip() or None,
        "armor_class": _integer(source.get("Armor Class")), "armor_class_text": source.get("Armor Class"), "max_hp": _integer(hp), "hit_points_text": hp, "hit_dice": hit_dice.group(1) if hit_dice else None,
        "speed": _speed(source.get("Speed")), "speed_text": source.get("Speed"), "abilities": {key.lower(): _integer(source.get(key)) for key in ("STR", "DEX", "CON", "INT", "WIS", "CHA")},
        "saving_throws": _bonuses(source.get("Saving Throws")), "skills": _bonuses(source.get("Skills")), "senses": source.get("Senses"), "languages": source.get("Languages"),
        "damage_resistances": resist, "damage_immunities": immune, "damage_vulnerabilities": vulnerable, "condition_immunities": condition_immune,
        "unsupported_defense_text": bad_resist + bad_immune + bad_vulnerable + bad_condition, "challenge_rating": (source.get("Challenge") or "").split(" ", 1)[0] or None,
        "attacks": attacks, "multiattack_slots": multiattack["slots"] if multiattack else [], "action_names": _names(action_text), "trait_names": _names(source.get("Traits")),
        "reaction_names": _names(reactions_text), "parry_ac_bonus": parse_parry_ac_bonus(reactions_text), "legendary_action_names": _names(source.get("Legendary Actions")),
        "source_traits": source.get("Traits"), "source_actions": action_text, "source_reactions": reactions_text, "source_legendary_actions": source.get("Legendary Actions"), "image_url": source.get("img_url"),
    }
...
def main() -> int:
    ...
    try:
        ...
        print(f"wrote {len(catalog)} monsters to {args.output}"); return 0
    except Exception as exc:
        logger.exception("2014 monster import failed: %s", exc); return 1
