...
from import_2014_attack_details import parse_on_hit_save_condition, parse_secondary_damage
...
def _attack(paragraph: str) -> dict | None:
    ...
    result = {"id": _slug(name), "name": name, "kind": kind, "attack_bonus": int(hit.group(1)), "damage": damage,
              "on_hit_damage": extras, "on_hit_save_effect": save_effect,
...
def _attacks(paragraph: str) -> list[dict]:
    ...
    base_id = attack["id"]
    shared = {**attack, "on_hit_damage": extras, "on_hit_save_effect": save_effect, "source_complete": True, "unsupported_text": None}
