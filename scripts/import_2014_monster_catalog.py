...
def _attack(paragraph: str) -> dict | None:
    text = _plain(paragraph); name_match = re.search(r"<strong>(.*?)</strong>", paragraph, re.I | re.S)
    hit = re.search(r"([+-]\d+) to hit", text, re.I)
    damage = re.search(r"Hit:\s*(\d+)\s*\((\d+)d(\d+)(?:\s*([+-])\s*(\d+))?\)\s*([A-Za-z]+) damage", text, re.I)
    if not name_match or not hit or not damage: return None
    ...
    average, count, size, sign, bonus, damage_type = damage.groups(); damage_type = damage_type.lower()
    if damage_type not in DAMAGE_TYPES: return None
    name = _plain(name_match.group(1)).rstrip("."); remainder = text[damage.end():].strip(" .")
...
def _attacks(paragraph: str) -> list[dict]:
    ...
    base_id = attack["id"]
    melee = {**attack, "id": f"{base_id}-melee", "kind": "melee", "source_complete": True}
    ranged = {**attack, "id": f"{base_id}-ranged", "kind": "ranged", "source_complete": True}
    return [melee, ranged]
...
def _record(source: dict) -> dict:
    ...
    return {
        ...
        "condition_immunities": condition_immune, "unsupported_defense_text": bad_resist + bad_immune + bad_vulnerable + bad_condition,
...
def main() -> int:
    ...
    parser.add_argument("source", type=Path); parser.add_argument("--output", type=Path, default=Path("data/monsters/2014/catalog.json")); args = parser.parse_args()
    try:
        catalog = [_record(item) for item in json.loads(args.source.read_text(encoding="utf-8"))]
        args.output.parent.mkdir(parents=True, exist_ok=True); args.output.write_text(json.dumps(catalog, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
