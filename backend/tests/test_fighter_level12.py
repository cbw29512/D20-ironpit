...
from app.content.fighter_level12_combat_profile import build_karnok_stoneward_level12_combat_profile
...
def test_fighter_level_twelve_snapshot_applies_constitution_asi() -> None:
    karnok = build_karnok_stoneward_level(12)
    ...
    assert karnok.saving_throw_bonuses["constitution"] == 9
...
def test_fighter_level_twelve_passes_profile_fingerprint_and_registry_gates() -> None:
    template = build_karnok_stoneward_level(12)
    profile = build_karnok_stoneward_level12_profile()
    combat_profile = build_karnok_stoneward_level12_combat_profile()
    ...
    assert registry[("fighter", 12, "canonical")] == (
        "Karnok Stoneward", "karnok-stoneward-l12",
    )
    assert registry[("fighter", 15, "canonical")] == ("Karnok Stoneward", "karnok-stoneward-l15")
