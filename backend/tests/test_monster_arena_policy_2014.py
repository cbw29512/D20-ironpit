...
def _monster(*, action_names: list[str] | None = None) -> CatalogMonster2014:
    try:
...
def test_arena_disabled_actions_are_not_certification_blockers() -> None:
    try:
        source = _monster(action_names=["Summon Fiend", "Teleport", "Plane Shift", "Banishment"])
...
def test_explicit_arena_traits_are_preserved_as_nonblocking_provenance() -> None:
    try:
        assert {"Earth Glide", "Siege Monster", "Spider Climb"} <= ARENA_OUT_OF_SCOPE_TRAITS_2014
