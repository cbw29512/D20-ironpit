...
from app.content.combat_build_choice_overlays import COMBAT_BUILD_CHOICE_OVERLAYS, FIGHTER_COMBAT_BUILD_CHOICES
from app.content.combat_engine_coverage import audit_build_capability_contract, audit_current_build_capabilities


def _build_capability_statuses() -> dict[str, str]:
    return {
...
def _build_statuses() -> dict[tuple[str, str], str]:
    active = {("fighter", "great-weapon"), ("fighter", "sword-shield")}
    return {
        key: "active" if key in active else "planned"
        for key in COMBAT_BUILD_CHOICE_OVERLAYS
    }


def test_current_build_overlays_accept_explicit_supported_blocked_and_arena_statuses() -> None:
    assert audit_current_build_capabilities(_build_capability_statuses()) == []
...
def test_planned_barbarian_builds_may_declare_blocked_masteries_without_activation() -> None:
    statuses = _build_capability_statuses()
    assert audit_current_build_capabilities(statuses) == []
    statuses.pop("cleave-mastery")
    issues = audit_current_build_capabilities(statuses)
    assert any("barbarian/great-weapon" in issue and "cleave-mastery" in issue for issue in issues)
