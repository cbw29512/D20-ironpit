...
def test_reviewed_neighbor_bleed_rows_end_at_their_real_stat_blocks() -> None:
    ...
    for name, (real_ending, spillover) in action_bleeds.items():
        row = rows[name]
    ...
    aboleth = rows["Aboleth"]
    assert str(aboleth["legendaryActions"]).endswith("Hit Points.")
    assert str(aboleth["rawText"]).endswith("Hit Points.")
    assert not str(aboleth["legendaryActions"]).endswith("Air Elemental")
    assert not str(aboleth["rawText"]).endswith("Air Elemental")
