...
from app.combat.saving_throws import (
    legal_save_action,
    resolve_save_action,
...
def test_saving_throws_do_not_use_attack_nat_one_or_nat_twenty_rules() -> None:
    state = _state()
    high_roll, high_success = resolve_saving_throw(
        state, "intelligence", 20, FixedDiceProvider([20]),
    )
    low_roll, low_success = resolve_saving_throw(
        state, "strength", 6, FixedDiceProvider([1]),
    )

    assert high_roll is not None and high_roll.total == 19
    assert high_success is False
    assert low_roll is not None and low_roll.total == 6
    assert low_success is True
...
def test_restrained_dexterity_save_has_disadvantage() -> None:
    ...
    roll, succeeded = resolve_saving_throw(
        state, "dexterity", 10, FixedDiceProvider([18, 2]),
    )

    assert roll is not None
    assert roll.mode is RollMode.DISADVANTAGE
    assert roll.selected_roll == 2
...
def test_rage_strength_save_has_advantage() -> None:
    ...
    roll, succeeded = resolve_saving_throw(
        state, "strength", 12, FixedDiceProvider([2, 10]),
    )
...
def test_unconscious_auto_fails_strength_and_dexterity_saves() -> None:
    ...
    strength_roll, strength_success = resolve_saving_throw(
        state, "strength", 1, FixedDiceProvider([20]),
    )
    dex_roll, dex_success = resolve_saving_throw(
        state, "dexterity", 1, FixedDiceProvider([20]),
    )
...
def test_constrict_failure_deals_damage_and_grapples() -> None:
    ...
    event = resolve_save_action(
        1, 1, snake, hero, action, 5, FixedDiceProvider([1, 1, 2, 3]),
    )
...
def test_constrict_success_deals_no_damage_or_grapple() -> None:
    setup = build_encounter_setup(EncounterSelection(
        hero_ids=["karnok-stoneward-l1"], monster_ids=["srd-constrictor-snake"], starting_distance_ft=5,
    ))
    hero, snake = setup.heroes[0], setup.monsters[0]
    ...
    event = resolve_save_action(
        1, 1, snake, hero, action, 5, FixedDiceProvider([10, 1, 1, 1]),
    )
