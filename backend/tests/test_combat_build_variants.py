...
def test_rogue_roles_are_owned_by_real_subclasses() -> None:
    assert {
        variant.id: variant.required_subclass_id
        for variant in combat_build_variants_for("rogue")
    } == {
        "duelist": "arcane-trickster",
        "dual-wield": "thief",
        "ranged": "assassin",
    }


def test_wizard_roles_are_owned_by_real_subclasses() -> None:
    assert {
        variant.id: variant.required_subclass_id
        for variant in combat_build_variants_for("wizard")
    } == {
        "fire-damage": "evoker",
        "frost-control": "illusionist",
        "mixed-arcane": "abjurer",
    }
...
def test_warlock_roles_are_owned_by_real_subclasses() -> None:
    assert {
        variant.id: variant.required_subclass_id
        for variant in combat_build_variants_for("warlock")
    } == {
        "blaster": "fiend-patron",
        "controller": "great-old-one-patron",
        "blade-hybrid": "celestial-patron",
    }
...
def test_cleric_roles_are_owned_by_real_domains() -> None:
    assert {
        variant.id: variant.required_subclass_id
        for variant in combat_build_variants_for("cleric")
    } == {
        "healer": "life-domain",
        "war-priest": "war-domain",
        "divine-offense": "light-domain",
    }
