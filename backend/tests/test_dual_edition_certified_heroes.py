...
def test_all_edition_registry_contains_exact_certified_2014_progressions() -> None:
    ...
    assert [key[1] for key, _ in monks] == list(range(1, 17))
    assert [key[1] for key, _ in paladins] == list(range(1, 13))
...
def test_arena_fingerprints_stay_2024_while_all_edition_registry_adds_2014() -> None:
    ...
    monk_ids = {f"kael-stillwater-2014-l{level}" for level in range(1, 17)}
    paladin_ids = {f"aurelia-brightshield-2014-l{level}" for level in range(1, 13)}
