...
def _member(combatant_id: str, position_ft: int, *, healer: bool = False):
    ...
    return EncounterCombatant(
        combatant_id=combatant_id, side="heroes", position_ft=position_ft,
...
def _enemy(position_ft: int = 10):
    return EncounterCombatant(
        combatant_id="enemy", side="monsters", position_ft=position_ft,
...
def test_cure_wounds_rescues_adjacent_zero_hp_ally_with_printed_level_slot() -> None:
    ...
    ally = _member("ally", 5)
    setup = EncounterSetup(heroes=[healer, ally], monsters=[_enemy()], hero_total_levels=2, monster_total_cr="1")
    ...
    event = resolve_healing(1, 1, healer, ally, action, FixedDiceProvider([8, 7]), turn_key)
    assert event.healing_roll is not None and event.healing_roll.total == 18
    assert event.hp_before == 0 and event.hp_after == min(18, ally.state.template.max_hp)
    assert ally.state.is_unconscious is False
    assert ally.state.death_save_successes == 0 and ally.state.death_save_failures == 0
    assert healer.state.action_available is False
    assert healer.state.spell_slot_expended_turn_key == turn_key
    assert next(item for item in healer.state.resources if item.id == "spell-slot-1").current_uses == 1


def test_cure_wounds_fails_closed_without_turn_key() -> None:
    healer = _member("healer", 0, healer=True)
    ally = _member("ally", 5)
...
def test_cure_wounds_cannot_reach_beyond_touch_formation_band() -> None:
    ...
    ally.state.current_hp = 1
    setup = EncounterSetup(heroes=[healer, ally], monsters=[_enemy()], hero_total_levels=2, monster_total_cr="1")
    assert choose_healing_action(healer, setup, "1:healer") is None
