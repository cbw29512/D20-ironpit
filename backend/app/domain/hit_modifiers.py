...
class HitModifierEffect(BaseModel):
    ...
    kind: Literal["attacks-against-advantage", "next-attack-made-disadvantage", "speed"]
    flat_bonus: int = 0
    ...
    def validate_payload(self) -> "HitModifierEffect":
        if self.kind in {"attacks-against-advantage", "next-attack-made-disadvantage"} and self.flat_bonus:
        ...
        if self.consume_on_attack_made and self.kind != "next-attack-made-disadvantage":
            raise ValueError("Only next-attack-made Disadvantage can be consumed by the target's attack.")
