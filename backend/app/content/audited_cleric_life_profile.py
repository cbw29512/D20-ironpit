...
def build_seraphine_dawnshield_level7_profile() -> CharacterBuildProfile:
    ...
    additions = [
        _class_feature(
            "blessed-strikes", "Blessed Strikes — Potent Spellcasting",
            notes="Canonical caster choice: add Wisdom modifier to damaging Cleric cantrips; Sacred Flame is 2d8 + Wisdom at level 7.",
        ),
        _class_feature(
            "cleric-combat-spells-4", "Level 4 Cleric Spells", combat=False,
            notes="Iron Pit simplicity policy: mandatory Aura of Life and Death Ward remain prepared; complex persistent level-4 effects are not preferred arena actions.",
        ),
        _class_feature(
            "prayer-of-healing", "Prayer of Healing", combat=False,
            notes="Prepared healing spell retained for RAW progression; 10-minute casting time keeps it outside arena turns.",
        ),
    ]
    data.update(
        feature_audits=[*data["feature_audits"], *(feature.model_dump() for feature in additions)],
        source_references=[
            *data["source_references"],
            "D&D Beyond Basic Rules 2024: Cleric level 7 — Blessed Strikes, 11 prepared spells, 4/3/3/1 spell slots",
...
def build_seraphine_dawnshield_level8_profile() -> CharacterBuildProfile:
    ...
    additions = [
        FeatureAudit(
            ...
            feature_name="Ability Score Improvement",
            source_reference="D&D Beyond Basic Rules 2024: Cleric Level 8; Feats — Ability Score Improvement",
            category="feat",
            combat_relevant=True,
            automated=True,
            notes="+1 Wisdom and +1 Charisma: WIS 19→20, CHA 14→15; spell save DC and Wisdom-based damage/healing increase by 1.",
        ),
        _class_feature(
            "guardian-of-faith", "Guardian of Faith", combat=False,
            notes="Prepared as the damage-side level-8 addition, but persistent positional guardian bookkeeping is intentionally outside the simple arena action set.",
        ),
    ]
    data.update(
        feature_audits=[*data["feature_audits"], *(feature.model_dump() for feature in additions)],
        source_references=[
            *data["source_references"],
            "D&D Beyond Basic Rules 2024: Cleric level 8 — Ability Score Improvement, 12 prepared spells, 4/3/3/2 spell slots",
