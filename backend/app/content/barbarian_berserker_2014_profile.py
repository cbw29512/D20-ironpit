...
def _advancements(level: int) -> list[AbilityIncrease]:
    ...
    if level >= 20:
        increases.extend([AbilityIncrease(ability="strength", amount=2), AbilityIncrease(ability="strength", amount=2),
                          AbilityIncrease(ability="constitution", amount=2), AbilityIncrease(ability="constitution", amount=2)])
    return increases


def _final_scores(base: AbilityScores, species: list[AbilityIncrease], advancements: list[AbilityIncrease]) -> AbilityScores:
    values = base.model_dump()
    for increase in [*species, *advancements]: values[increase.ability] += increase.amount
    return AbilityScores(**values)


def _feature_audits(level: int) -> list[FeatureAudit]:
    barbarian = "D&D Basic Rules 2014: Barbarian"; berserker = "D&D Basic Rules 2014: Path of the Berserker"
    audits = [
        _audit("human-ability-increase", "Human Ability Score Increase", "species", source="D&D Basic Rules 2014: Human"),
        _audit("rage", "Rage", "class", source=barbarian), _audit("unarmored-defense", "Unarmored Defense", "class", source=barbarian),
    ...
    if level >= 2: audits.extend([_audit("reckless-attack", "Reckless Attack", "class", source=barbarian), _audit("danger-sense", "Danger Sense", "class", source=barbarian)])
    if level >= 3: audits.append(_audit("frenzy", "Frenzy", "subclass", source=berserker))
    if level >= 5: audits.extend([_audit("extra-attack", "Extra Attack", "class", source=barbarian), _audit("fast-movement", "Fast Movement", "class", source=barbarian)])
...
def _compile_rokhan_stonefury_2014_profile(level: int) -> CharacterBuildProfile:
    if level not in range(1, 21): raise ValueError("2014 Rokhan profile progression covers levels 1 through 20.")
    base = _base_scores(); species = _species_increases(); advancements = _advancements(level)
    return CharacterBuildProfile(
        id=f"build-rokhan-stonefury-2014-l{level}", template_id=f"rokhan-stonefury-2014-l{level}", character_name="Rokhan Stonefury",
        class_id="barbarian", class_name="Barbarian", level=level, ruleset="2014", subclass_id="path-berserker" if level >= 3 else None,
        subclass_name="Path of the Berserker" if level >= 3 else None, build_id="berserker-greataxe", species_id="human", species_name="Human",
        background_id="soldier", background_name="Soldier", base_ability_scores=base, species_increases=species,
        advancement_increases=advancements, final_ability_scores=_final_scores(base, species, advancements),
        ability_score_maximums={"strength": 24, "constitution": 24} if level >= 20 else {}, class_equipment_option="package",
        class_equipment=["Greataxe", "Two Handaxes", "Explorer's Pack", "Four Javelins"], background_equipment_option="package",
        background_equipment=["Rank Insignia", "Trophy", "Gaming Set", "Common Clothes", "10 gp"],
        skill_proficiencies=["Athletics", "Intimidation", "Perception", "Survival"], weapon_masteries=[], combat_loadout_kind="two-handed",
        feature_audits=_feature_audits(level), source_references=["D&D Basic Rules 2014: Human", "D&D Basic Rules 2014: Barbarian",
        "D&D Basic Rules 2014: Path of the Berserker", "D&D Basic Rules 2014: Soldier", "D&D Basic Rules 2014: Equipment"])


def build_rokhan_stonefury_2014_profile(level: int) -> CharacterBuildProfile:
    try:
        if level not in range(1, 21): raise ValueError("2014 Rokhan profile certification covers levels 1 through 20.")
