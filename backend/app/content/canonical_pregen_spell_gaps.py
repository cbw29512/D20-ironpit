...
def canonical_pregen_spell_package_gaps() -> tuple[SpellPackageGap, ...]:
    """Report every canonical caster level whose deterministic spell package is incomplete."""
    gaps: list[SpellPackageGap] = []
    for class_id in sorted(CANONICAL_SPELLS):
        for level in range(1, 21):
            try:
                build_class_spell_package(class_id, level)
...
def first_spell_package_gap_by_class() -> dict[str, SpellPackageGap | None]:
    first = {class_id: None for class_id in sorted(CANONICAL_SPELLS)}
