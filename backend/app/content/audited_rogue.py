...
def _scores(level: int) -> AbilityScores:
    """Return the canonical ability-score state for Mara at the requested level."""
...
def _max_hp(level: int) -> int:
    """Apply the Rogue hit-die progression, including the level-4 CON increase."""
    hp = 10 + 7 * (level - 1)
...
def build_mara_quickstep_level(level: int) -> CombatantTemplate:
    ...
    # Levels above four require additional shared reaction/Cunning Strike support.
    # Keep the compiler fail-closed rather than emitting a partially modeled hero.
    if level > 4:
    ...
    shortsword = build_mara_shortsword_attack().model_copy(
        update={
            "attack_bonus": row.proficiency_bonus + dexterity_mod,
            "damage_bonus": dexterity_mod,
        }
    )
    shortbow = build_mara_shortbow_attack().model_copy(
        update={
            "attack_bonus": row.proficiency_bonus + dexterity_mod,
            "damage_bonus": dexterity_mod,
        }
    ...
    return CombatantTemplate(
        ...
        source=(
            f"D&D Beyond Basic Rules 2024: Rogue {level}, Thief, Orc, Soldier, "
            "Savage Attacker, Leather Armor, Shortsword, Shortbow, Vex"
        ),
    )
