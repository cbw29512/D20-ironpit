...
def build_rules_coverage() -> RulesCoverageReport:
    try:
        entries = [
            ...
            RuleCoverageEntry(id="light-weapons", name="Light weapon extra attack", status=I, notes="A Light-weapon Attack-action attack can enable one different Light-weapon extra attack; Bonus Action cost, once-per-turn use, and ability-modifier damage rules are implemented."),
            RuleCoverageEntry(id="two-weapon-fighting", name="Two-Weapon Fighting style", status=I, notes="The style restores the ability modifier to damage for the Light-property extra attack."),
            RuleCoverageEntry(id="nick", name="Nick mastery", status=I, notes="A mastered Nick weapon can make the Light-property extra attack inside the Attack action instead of spending the Bonus Action; it does not grant a second Light extra attack."),
