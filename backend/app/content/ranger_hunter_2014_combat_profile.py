...
def build_rowan_2014_combat_profile(level: int) -> PregenCombatProfile:
    try:
        if level not in range(1, 11):
            raise ValueError("2014 Rowan combat fingerprint currently covers levels 1 through 10.")
...
def build_rowan_2014_combat_profiles() -> list[PregenCombatProfile]:
    return [build_rowan_2014_combat_profile(level) for level in range(1, 11)]
