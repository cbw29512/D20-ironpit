...
def build_rowan_2014_combat_profile(level: int) -> PregenCombatProfile:
    try:
        if level not in range(1, 19):
            raise ValueError("2014 Rowan combat fingerprint currently covers levels 1 through 18.")
...
def build_rowan_2014_combat_profiles() -> list[PregenCombatProfile]:
    return [build_rowan_2014_combat_profile(level) for level in range(1, 19)]
