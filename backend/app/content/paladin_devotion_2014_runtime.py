...
from app.content.paladin_devotion_2014_effect_removal import cleansing_touch_2014, dispel_magic_2014
...
def _resources(level: int) -> list[ResourceDefinition]:
    ...
    if level >= 14:
        resources.append(ResourceDefinition(
            ...
            max_uses=max(1, _scores(level).modifier("charisma")),
        ))
    return resources
...
def build_aurelia_brightshield_2014(level: int) -> CombatantTemplate:
    try:
        if level not in range(1, 15):
            raise ValueError("2014 Devotion Paladin runtime covers levels 1 through 14.")
        ...
        return CombatantTemplate(
            ...
            effect_removal_actions=(
                ([cleansing_touch_2014()] if level >= 14 else [])
                + ([dispel_magic_2014()] if level >= 9 else [])
            ),
