...
def build_thalen_2014_combat_profile(level: int) -> PregenCombatProfile:
    try:
        if level not in range(1, 11):
            raise ValueError("2014 Thalen combat fingerprint currently covers levels 1 through 10.")
        ...
        return PregenCombatProfile(
            ...
            armor_class=15,
            max_hp={1: 10, 2: 17, 3: 24, 4: 31, 5: 38, 6: 45, 7: 52, 8: 59, 9: 66, 10: 73}[level],
            speed_ft=35,
            skill_bonuses=(
            ...
            resources={
                ...
                10: (("spell-slot-1", 4), ("spell-slot-2", 3), ("spell-slot-3", 3), ("spell-slot-4", 3), ("spell-slot-5", 2), ("wild-shape", 2)),
            }[level],
...
def build_thalen_2014_combat_profiles() -> list[PregenCombatProfile]:
    return [build_thalen_2014_combat_profile(level) for level in range(1, 11)]
