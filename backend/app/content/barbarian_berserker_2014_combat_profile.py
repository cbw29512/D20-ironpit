...
from app.content.barbarian_berserker_2014_profile import _compile_rokhan_stonefury_2014_profile
...
def _compile_rokhan_2014_combat_profile(level: int) -> PregenCombatProfile:
    ...
    return PregenCombatProfile(
        ...
        max_hp=fixed_hit_points(level, 12, scores.modifier("constitution")), speed_ft=40 if level >= 5 else 30,
        skill_bonuses=(("athletics", scores.modifier("strength") + pb), ("acrobatics", scores.modifier("dexterity"))),
        attacks=attacks, weapon_masteries=(), resources=resources, unlimited_resources=("rage",) if level >= 20 else (),
...
def build_rokhan_2014_combat_profile(level: int) -> PregenCombatProfile:
    try:
        if level not in range(1, 21): raise ValueError("2014 Rokhan combat-profile certification covers levels 1 through 20.")
