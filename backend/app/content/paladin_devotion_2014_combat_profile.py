...
def build_aurelia_brightshield_2014_combat_profile(level: int) -> PregenCombatProfile:
    try:
        if level not in range(1, 18):
            raise ValueError("2014 Aurelia combat fingerprint covers levels 1 through 17.")
...
def build_aurelia_2014_combat_profiles() -> list[PregenCombatProfile]:
    try:
        return [build_aurelia_brightshield_2014_combat_profile(level) for level in range(1, 18)]
    except Exception:
