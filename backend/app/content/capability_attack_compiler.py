...
from app.domain.capability_effects import ConditionEffectDefinition, DamageEffectDefinition, GrappleEffectDefinition, MaxHpReductionEffectDefinition, ProneEffectDefinition
...
def compile_attack(definition: AttackCapabilityDefinition) -> WeaponAttack:
    ...
    weapon = Weapon(
        ...
        projectile=definition.projectile, mastery_property=definition.mastery_property, light=definition.light,
        finesse=definition.finesse, heavy=definition.heavy, two_handed=definition.two_handed, versatile=definition.versatile,
    )
    on_hit: list[OnHitDamage] = []; on_hit_modifiers: list[HitModifierEffect] = []
    conditional: list[ConditionalDamage] = []; max_hp_reduction = None; prone_size = None; control = None
    for effect in definition.effects:
        if isinstance(effect, DamageEffectDefinition):
            if effect.trigger == "on_hit":
                on_hit.append(OnHitDamage(source=effect.source, dice_count=effect.dice.count, dice_size=effect.dice.size,
                                          damage_bonus=effect.dice.bonus, damage_type=effect.damage_type))
            else:
                conditional.append(ConditionalDamage(trigger=effect.trigger, mode=effect.mode,
                                                     dice_count=effect.dice.count, dice_size=effect.dice.size,
                                                     damage_bonus=effect.dice.bonus, damage_type=effect.damage_type))
        elif isinstance(effect, MaxHpReductionEffectDefinition):
            if max_hp_reduction is not None: raise UnsupportedCapabilityError("An attack can have only one maximum-HP reduction rider.")
            max_hp_reduction = MaxHpReductionRider(damage_type=effect.damage_type)
        elif isinstance(effect, ProneEffectDefinition): prone_size = effect.max_target_size
        elif isinstance(effect, HitModifierEffect): on_hit_modifiers.append(effect)
        elif isinstance(effect, (GrappleEffectDefinition, ConditionEffectDefinition)): control = _compile_control(effect)
        else: raise UnsupportedCapabilityError(f"Unsupported attack effect: {effect!r}")
    return WeaponAttack(
        id=definition.id, weapon=weapon, attack_bonus=definition.attack_bonus, damage_bonus=damage.bonus if damage else 0,
