...
from app.content.capability_attack_control_compiler import UnsupportedAttackControlError, compile_control, merge_controls
from app.content.capability_attack_save_compiler import compile_hit_save
...
from app.domain.capability_effects import (
    ConditionEffectDefinition, DamageEffectDefinition, GrappleEffectDefinition,
    HitSavingThrowEffectDefinition, MaxHpReductionEffectDefinition, ProneEffectDefinition,
)
...
def compile_attack(definition: AttackCapabilityDefinition) -> WeaponAttack:
    try:
        ...
        weapon = Weapon(
            id=definition.weapon_id or definition.id, name=definition.name, attack_kind=definition.attack_kind,
            dice_count=damage.count if damage else 0, dice_size=damage.size if damage else 2,
            damage_type=definition.damage_type, animation=definition.animation, reach_ft=definition.reach_ft,
            normal_range_ft=definition.normal_range_ft, long_range_ft=definition.long_range_ft,
            projectile=definition.projectile, mastery_property=definition.mastery_property,
            light=definition.light, finesse=definition.finesse, heavy=definition.heavy,
            two_handed=definition.two_handed, versatile=definition.versatile,
        )
            ...
            if isinstance(effect, DamageEffectDefinition):
                if effect.trigger == "on_hit":
                    ...
                    conditional.append(ConditionalDamage(
                        trigger=effect.trigger, mode=effect.mode, dice_count=effect.dice.count,
                        dice_size=effect.dice.size, damage_bonus=effect.dice.bonus, damage_type=effect.damage_type,
                    ))
                ...
                hit_save = compile_hit_save(effect)
        ...
        return WeaponAttack(
            id=definition.id, weapon=weapon, attack_bonus=definition.attack_bonus,
            damage_bonus=damage.bonus if damage else 0, attack_ability=definition.attack_ability,
            attack_ability_modifier=definition.attack_ability_modifier, resource_id=definition.resource_id,
            resource_cost=definition.resource_cost, rage_eligible=definition.rage_eligible,
            fixed_damage=definition.fixed_damage, conditional_damage=conditional,
            conditional_attack_advantage=definition.conditional_attack_advantage,
            on_hit_damage=on_hit, on_hit_modifier_effects=on_hit_modifiers, on_hit_saving_throw=hit_save,
            max_hp_reduction_on_hit=max_hp_reduction, ability_score_reduction_on_hit=ability_reduction,
            attachment_on_hit=attachment, knocks_prone_max_size=prone_size,
            control_effect=merge_controls(controls), charge_profile=definition.charge_profile,
