...
def build_karnok_stoneward_level15_profile() -> CharacterBuildProfile:
    try:
        ...
        superior_critical = FeatureAudit(
            ...
            automated=True,
            notes=(
                "Karnok scores a Critical Hit on an attack roll of 18–20. "
                "The shared critical_hit_minimum progression field already resolves this rule."
            ),
        )
        data.update(
...
def build_karnok_stoneward_level16_profile() -> CharacterBuildProfile:
    try:
        ...
        ability_score_improvement = FeatureAudit(
            ...
            automated=True,
            notes=(
                "Deterministic progression choice: +2 Dexterity, DEX 15→17. "
                "The authoritative Fighter table updates initiative, ranged attacks, "
                "Dexterity checks, saving throws, and the sixth Weapon Mastery."
            ),
        )
        data.update(
            feature_audits=[*data["feature_audits"], ability_score_improvement.model_dump()],
...
def build_karnok_stoneward_level17_profile() -> CharacterBuildProfile:
    try:
        ...
        resource_progression = FeatureAudit(
            feature_id="fighter-resource-progression-l17",
            feature_name="Action Surge and Indomitable Uses",
            source_reference=(
                "D&D Beyond Basic Rules 2024: Fighter Level 17 — "
                "Action Surge and Indomitable"
            ),
            category="class",
            combat_relevant=True,
            automated=True,
            notes=(
                "Action Surge increases to two uses and Indomitable to three uses. "
                "The shared resource system reads these counts from the canonical Fighter table."
            ),
        )
        data.update(
            feature_audits=[*data["feature_audits"], resource_progression.model_dump()],
            source_references=[
                *data["source_references"],
