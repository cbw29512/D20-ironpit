...
def build_seraphine_2014_combat_profile(level: int) -> PregenCombatProfile:
    try:
        if level not in range(1, 16):
            raise ValueError("2014 Seraphine combat fingerprint is currently certified through level 15.")
...
def build_seraphine_2014_combat_profiles() -> list[PregenCombatProfile]:
    try:
        return [build_seraphine_2014_combat_profile(level) for level in range(1, 16)]
    except Exception:
