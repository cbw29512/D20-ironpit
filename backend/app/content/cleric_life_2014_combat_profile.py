...
def build_seraphine_2014_combat_profile(level: int) -> PregenCombatProfile:
    try:
        if level not in range(1, 12):
            raise ValueError("2014 Seraphine combat fingerprint is currently certified through level 11.")
...
def build_seraphine_2014_combat_profiles() -> list[PregenCombatProfile]:
    try:
        return [build_seraphine_2014_combat_profile(level) for level in range(1, 12)]
    except Exception:
