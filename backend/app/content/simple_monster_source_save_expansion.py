...
_POINT_TIMED = re.compile(
    rf"^(?P<name>.+?)\. (?P<ability>{_ABILITY}) Saving Throw: DC (?P<dc>\d+), each (?:creature|enemy) in a (?P<radius>\d+)-foot-radius Sphere centered on a point .*?within (?P<range>\d+) feet\. "
...
_POINT_DAMAGE = re.compile(
    rf"^(?P<name>.+?)\. (?P<ability>{_ABILITY}) Saving Throw: DC (?P<dc>\d+), each (?:creature|enemy) in a (?P<radius>\d+)-foot-radius Sphere centered on a point .*?within (?P<range>\d+) feet\. "
    rf"Failure: {_DICE}\. Success: Half damage(?: only)?\.$", re.I,
)
_EMANATION_DAMAGE_TIMED = re.compile(
    rf"^(?P<name>.+?)\. (?P<ability>{_ABILITY}) Saving Throw: DC (?P<dc>\d+), each (?:creature|enemy) in a (?P<radius>\d+)-foot Emanation originating from .+?\. "
    rf"Failure: {_DICE}, and the target has the (?P<condition>{_CONDITION}) condition until the (?P<edge>start|end) of (?P<owner>.+?) next turn\.$", re.I,
)
...
def _timing(row: dict[str, object], edge: str, owner: str) -> str:
    ...
    owner_words = set(re.findall(r"[a-z]+", normalized))
    source_words = set(re.findall(r"[a-z]+", str(row["name"]).lower())) - {"adult", "ancient", "young", "of"}
    if owner_words & source_words:
...
def _damage(match: re.Match[str]) -> dict[str, object]:
    ...
    return {"damage": {"count": int(match.group("count")), "size": int(match.group("size")), "bonus": bonus}, "damage_type": match.group("type").lower()}
...
def _area_result(row: dict[str, object], heading: str, match: re.Match[str], shape: str, size_group: str) -> dict[str, object]:
    size = int(match.group(size_group)); result = _base(row, heading, match, int(match.group("range")) if "range" in match.groupdict() else size)
    result["area"] = {"shape": shape, "size_ft": size}; return result
...
def parse_expanded_save(row: dict[str, object], heading: str, block: str) -> dict[str, object] | None:
    ...
    if match:
        result = _base(row, heading, match, int(match.group("range"))); result.update(_damage(match)); result["success_damage"] = "half" if match.group("success").lower().startswith("half damage") else "none"; result["failure_conditions"] = [_condition(match, timing=_timing(row, match.group("edge"), match.group("owner")))]; return result
    match = _SIZE_PRONE.fullmatch(block)
    if match:
        result = _base(row, heading, match, int(match.group("range"))); result.update(_damage(match)); result["failure_conditions"] = [_condition(match, max_size=match.group("max_size"))]; return result
    match = _SPACE_DAMAGE_PRONE.fullmatch(block)
    if match:
        result = _base(row, heading, match, 0); result.update(_damage(match)); result["target_max_size"] = match.group("max_size").lower(); result["success_damage"] = "half"; result["failure_conditions"] = [_condition(match, max_size=match.group("max_size"))]; return result
    match = _CONE_PRONE.fullmatch(block)
    if match:
        result = _area_result(row, heading, match, "cone", "length"); result["failure_conditions"] = [_condition(match)]; return result
    match = _CONE_TIMED.fullmatch(block)
    if match:
        result = _area_result(row, heading, match, "cone", "length"); result["failure_conditions"] = [_condition(match, timing=_timing(row, match.group("edge"), match.group("owner")))]; return result
    match = _POINT_TIMED.fullmatch(block)
    if match:
        result = _area_result(row, heading, match, "radius", "radius"); result.update(_damage(match)); result["failure_conditions"] = [_condition(match, timing=_timing(row, match.group("edge"), match.group("owner")))]; return result
    match = _POINT_DAMAGE.fullmatch(block)
    if match:
        result = _area_result(row, heading, match, "radius", "radius"); result.update(_damage(match)); result["success_damage"] = "half"; return result
    match = _EMANATION_DAMAGE_TIMED.fullmatch(block)
    if match:
        result = _area_result(row, heading, match, "radius", "radius"); result.update(_damage(match)); result["failure_conditions"] = [_condition(match, timing=_timing(row, match.group("edge"), match.group("owner")))]; return result
    match = _CONE_DAMAGE_SCOPED.fullmatch(block)
    if match:
        result = _area_result(row, heading, match, "cone", "length"); result.update(_damage(match)); result["success_damage"] = "half"; return result
    return None