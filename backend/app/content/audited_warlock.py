...
def build_varek_ashenmark_level(level: int) -> CombatantTemplate:
    ...
    features = canonical_combat_features("warlock", level)
    ...
    return CombatantTemplate(
        ...
        visual=VisualLoadout(
            armor="leather-armor",
            main_hand="pact-longsword",
            off_hand="arcane-focus-orb",
            body_style="humanoid",
        ),
        resources=[
            ResourceDefinition(
                id=f"pact-slot-{row.pact_slot_level}",
                name=f"Level {row.pact_slot_level} Pact Magic Slot",
