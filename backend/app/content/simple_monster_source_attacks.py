...
_ATTACK = re.compile(
    rf"\b(Melee|Ranged) Attack Roll:\s*([+-]?\d+)(?:\s*\([^)]*\))?,\s*"
    rf"(?:(?:reach\s+(\d+)\s*ft\.?)|(?:range\s+(\d+)\s*/\s*(\d+)\s*ft\.?))\s*{_DAMAGE}", re.I,
)
_HYBRID = re.compile(
    rf"\bMelee or Ranged Attack Roll:\s*([+-]?\d+)(?:\s*\([^)]*\))?,\s*"
    rf"reach\s+(\d+)\s*ft\.?\s+or\s+range\s+(\d+)\s*/\s*(\d+)\s*ft\.?\s*{_DAMAGE}", re.I,
)
