...
def build_mara_2014_combat_profiles() -> list[PregenCombatProfile]:
    ...
    try:
        return [build_mara_2014_combat_profile(level) for level in range(1, 14)]
    except Exception:
        logger.exception("Failed to compile 2014 Mara combat-profile progression through level 13")
        raise
