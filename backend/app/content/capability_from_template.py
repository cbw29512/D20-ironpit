...
def _attack(attack: WeaponAttack) -> dict[str, object]:
    ...
    effects: list[dict[str, object]] = [
        {
            "kind": "damage", "source": extra.source,
...
def _save(action) -> dict[str, object]:
    result: dict[str, object] = {
        ...
        "success_damage": action.success_damage, "magical": action.magical,
        "resource_id": action.resource_id, "resource_cost": action.resource_cost,
        "area_slots": action.area_slots, "priority": action.priority, "animation": action.animation,
    }
...
def definition_from_template(template: CombatantTemplate) -> CombatantDefinition:
    ...
    data = template.model_dump(mode="json", exclude={
        "weapon_attack", "alternate_weapon_attacks", "attack_action", "saving_throw_actions",
    })
