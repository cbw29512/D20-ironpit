...
def build_mara_quickstep_level7_profile() -> CharacterBuildProfile:
    """Level 7 inherits level 6, then adds Evasion and Reliable Talent."""
    try:
            ...
            feature_audits=[
                *data["feature_audits"],
                _feature(
                    ...
                    notes="Reuses the shared Dexterity-save half-damage Evasion resolver.",
                ).model_dump(),
                _feature(
                    "reliable-talent", "Reliable Talent", "class",
                    combat_relevant=True, automated=True,
                    notes=(
                        "Uses the shared proficient-skill d20 floor. In the current arena it applies "
                        "to Mara's Athletics/Acrobatics grapple-escape checks."
                    ),
                ).model_dump(),
            ],
            source_references=[
