...
_PROFILES = {
    "Young Black Dragon": (7, 2, 4, 4, 1, 6, "acid", "Acid Breath", "dexterity", 14, 14, 6, "acid", "line", 30, 5),
    "Young Blue Dragon": (9, 2, 6, 5, 1, 10, "lightning", "Lightning Breath", "dexterity", 16, 10, 10, "lightning", "line", 60, 5),
    "Young Green Dragon": (7, 2, 6, 4, 2, 6, "poison", "Poison Breath", "constitution", 14, 12, 6, "poison", "cone", 30, None),
    "Young Red Dragon": (10, 2, 6, 6, 1, 6, "fire", "Fire Breath", "dexterity", 17, 16, 6, "fire", "cone", 30, None),
    "Young White Dragon": (7, 2, 4, 4, 1, 4, "cold", "Cold Breath", "constitution", 15, 9, 8, "cold", "cone", 30, None),
}
...
def _build(name: str) -> CombatantTemplate:
    row = _row(name)
    (
        attack_bonus, rend_count, rend_size, damage_bonus, extra_count, extra_size, extra_type,
    ...
    rend = WeaponAttack(
        ...
        weapon=Weapon(
            ...
            dice_count=rend_count, dice_size=rend_size, damage_type=DamageType.SLASHING,
            animation="claw", reach_ft=10,
        ),
    ...
    return CombatantTemplate(
        ...
        challenge_rating=str(row["challenge"]).split()[0], kind="monster", size=CreatureSize.LARGE,
        ...
        attack_action=AttackActionDefinition(
            ...
            slots=[AttackActionSlot(attack_ids=[rend.id]) for _ in range(3)],
        ),
...
def build_recharge_young_dragons() -> list[CombatantTemplate]:
