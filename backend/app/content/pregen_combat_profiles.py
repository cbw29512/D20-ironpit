...
def build_pregen_combat_profiles() -> dict[str, PregenCombatProfile]:
    from app.content.rogue_combat_fingerprint import build_mara_quickstep_combat_profile; from app.content.warlock_combat_fingerprint import build_varek_ashenmark_combat_profile
    profiles = [
        *(_karnok_profile(level) for level in range(1, 18)),
        *(_rokhan_profile(level) for level in range(1, 9)),
        *(_seraphine_profile(level) for level in range(1, 5)),
        *(build_mara_quickstep_combat_profile(level) for level in range(1, 3)), build_varek_ashenmark_combat_profile(),
    ]
