...
from app.domain.models import BattleResult

logging.basicConfig(level=os.getenv("LOG_LEVEL", "INFO"))
logger = logging.getLogger(__name__)

app = FastAPI(title="Iron Pit API", version="0.1.0")
origins = [item.strip() for item in os.getenv("ALLOWED_ORIGINS", "http://localhost:8080,http://127.0.0.1:8080").split(",") if item.strip()]
app.add_middleware(
    CORSMiddleware,
...
def create_demo_battle() -> BattleResult:
    try:
        return run_duel(build_demo_fighter(), build_goblin_warrior(), SecureDiceProvider())
    except Exception as exc:
