...
def resolve_start_turn_recharges(
    ...
    """Roll only expended Recharge resources at the source's start-of-turn window."""
    try:
        ...
        for definition in state.template.resources:
            ...
            if runtime.current_uses >= runtime.max_uses:
                continue
            ...
            threshold = (
                f"Recharge {rule.minimum_roll}"
                if rule.minimum_roll == rule.die_size
                else f"Recharge {rule.minimum_roll}-{rule.die_size}"
            )
            result = (
                f"{definition.name} recharges ({before}->{after})."
                if succeeded
                else f"{definition.name} remains expended."
            )
            events.append(BattleEvent(
                ...
                animation="recharge",
                description=f"{state.template.name} rolls {rolled} for {threshold}. {result}",
                audit=EventAudit(steps=[
                    AuditStep(
                        phase=AuditPhase.ROLL,
                        kind="roll",
                        label=f"{definition.name} {threshold}: {rolled}",
                    ),
                    AuditStep(
