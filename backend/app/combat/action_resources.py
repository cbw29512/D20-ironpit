...
class ResourceAction(Protocol):
    name: str
    resource_id: str | None
    resource_cost: int


def resource_available(state: CombatantState, action: ResourceAction) -> bool:
...
def consume_resource(state: CombatantState, action: ResourceAction) -> int | None:
    if action.resource_id is None:
        return None
    resource = next((item for item in state.resources if item.id == action.resource_id), None)
