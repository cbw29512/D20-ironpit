...
def saving_throw_mode(state: CombatantState, ability: str, context: SavingThrowContext | None = None) -> RollMode:
    try:
        advantage = (
        ...
        disadvantage = (
            ...
            + int(state.exhaustion_level_2014 >= 3)
        )
...
def resolve_saving_throw(
    state: CombatantState, ability: str, dc: int, dice: DiceProvider,
    ...
    try:
        ...
        roll = apply_d20_bonus_dice(
            ...
            roll_d20(dice, state.template.saving_throw_bonuses[ability], saving_throw_mode(state, ability, context)), dice,
        )
        if roll.total < dc:
