...
def resolve_combat_turn(
    ...
    try:
        ...
        if forced_retreat_active(attacker.state):
            ...
            return finish_turn(
                events, sequence + 1, round_number, attacker, setup, dice, turn_key, allow_surge=False,
            )
        ...
        if should_escape_grapple(attacker.state):
            ...
            return finish_turn(events, sequence + 1, round_number, attacker, setup, dice, turn_key)
        ...
        priority_events, sequence, handled = resolve_pre_movement_offense(
        ...
        priority_events, sequence, handled = resolve_post_movement_offense(
            sequence, round_number, attacker, setup, turn_key, dice,
        )
        events.extend(priority_events)
        if handled:
