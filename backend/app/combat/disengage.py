...
def take_disengage(
    ...
    """Spend the Action so the actor's movement does not provoke OAs this turn."""
    try:
        ...
        return BattleEvent(
            ...
            animation="disengage",
            description=(
                f"{actor.template.name} takes the Disengage action; its movement "
                "does not provoke Opportunity Attacks this turn."
            ),
        )
    except ValueError:
        raise
