...
from app.combat.regeneration import regeneration_defers_death
...
def _combatant_defeated(member: EncounterCombatant) -> bool:
    ...
    if regeneration_defers_death(state) and state.current_hp <= 0 and not state.is_dead:
        return False
...
def resolve_encounter_outcome(setup: EncounterSetup) -> EncounterOutcome:
    """Return the deathmatch outcome while respecting deferred regenerative death."""
    try:
