...
def attack_roll_condition_sources(
    ...
    advantage = 0
    disadvantage = 0
    if has_condition(attacker, BLINDED_EFFECT_ID): disadvantage += 1
    if has_condition(attacker, FRIGHTENED_EFFECT_ID): disadvantage += 1
    if PRONE_EFFECT_ID in attacker.active_effect_ids: disadvantage += 1
    if RESTRAINED_EFFECT_ID in attacker.active_effect_ids: disadvantage += 1
    if POISONED_EFFECT_ID in attacker.active_effect_ids: disadvantage += 1
    if target_id is not None: disadvantage += grapple_attack_disadvantage(attacker, target_id)
    if (
    ...
    if attacks_have_advantage_against(defender): advantage += 1
    if RESTRAINED_EFFECT_ID in defender.active_effect_ids: advantage += 1
    if PRONE_EFFECT_ID in defender.active_effect_ids:
        if distance_ft <= 5: advantage += 1
        else: disadvantage += 1
...
def apply_hit_conditions(
    ...
    """Apply declarative persistent effects and modifiers after a successful hit."""
    ...
    if maximum is not None and size_at_most(defender.template.size, maximum) and not condition_is_immune(defender, PRONE_EFFECT_ID):
    ...
    applied.extend(apply_persistent_effects(
        defender,
        attack.ordered_persistent_effects(),
        source_id=source_id,
...
def stand_from_prone(state: CombatantState) -> int:
    ...
    if PRONE_EFFECT_ID not in state.active_effect_ids or speed <= 0 or speed_is_zero(state): return 0
