...
def select_obscure_die_cost(
    ...
    """Prefer the supported Devious Strike Obscure option when it can affect the target."""
...
def select_canonical_die_cost(
    ...
    """Choose one deterministic supported Cunning Strike option for automated arena play."""
...
def resolve_obscure(
    ...
    """Resolve Devious Strike: Obscure after Sneak Attack damage is dealt."""
    ...
    level = attacker.template.level
    if scores is None or level is None:
        raise ValueError("Devious Strike requires certified ability scores and level.")
    dc = 8 + scores.modifier("dexterity") + proficiency_bonus(level)
    save_roll, succeeded = resolve_saving_throw(defender, "dexterity", dc, dice)
    applied = False
    if not succeeded:
