...
from app.combat.modifier_stack import add_modifier, apply_d20_bonus_dice, attacks_against_advantage_sources, consume_attacks_against_advantage, effective_armor_class
...
def resolve_spell_attack(
    ...
    if distance > spell.range_ft: raise ValueError(f"{spell.name} target is out of range.")
    resource = _slot_resource(caster, spell, turn_key)
    if spell.level > 0 and resource is None: raise ValueError(f"No level {spell.level} spell slot remains for {spell.name}.")
    condition_advantage, condition_disadvantage = attack_roll_condition_sources(caster.state, target.state, distance, target.combatant_id)
    mode = resolve_roll_mode(condition_advantage + attacks_against_advantage_sources(target.state), condition_disadvantage + int(close_ranged_threat_exists(caster, setup)))
    attack_roll = apply_d20_bonus_dice(caster.state, ModifierKind.ATTACK_ROLL_BONUS_DIE, roll_d20(dice, spell.attack_bonus, mode), dice)
    ...
    natural = attack_roll.selected_roll or 0; target_ac = effective_armor_class(target.state)
    ...
    if hit:
        ...
        if target.state.is_alive and not target.state.is_dead:
            for index, effect in enumerate(spell.on_hit_modifier_effects):
                add_modifier(target.state, build_spell_modifier(caster.combatant_id, target.combatant_id, spell.id, effect, index, round_number=round_number))
    ...
    return BattleEvent(
        ...
        animation=spell.animation, description=f"{caster.state.template.name}: {outcome} with {spell.name}.",
    )
