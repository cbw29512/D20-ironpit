...
def resolve_divine_spark(
    ...
    *,
    ...
    dice_count = cleric.state.template.progression_features.feature_dice_counts.get(DIVINE_SPARK, 1)
    maximize = healing and healing_is_maximized(target.state, cleric.state, DIVINE_SPARK)
    rolls = [8 for _ in range(dice_count)] if maximize else [dice.roll(8) for _ in range(dice_count)]
    modifier = _wisdom_modifier(cleric, save_dc)
    total = sum(rolls) + modifier
    notation = f"{dice_count}d8+{modifier}"
    if healing:
    ...
    component = DamageRollComponent(
        source="Divine Spark", notation=notation, rolls=rolls, modifier=modifier,
    ...
    return BattleEvent(
        ...
        damage_roll=DiceRoll(notation=notation, rolls=rolls, modifier=modifier, total=applied_total),
