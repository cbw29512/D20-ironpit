...
def next_attack_made_disadvantage_sources(state: CombatantState) -> int:
    return sum(1 for item in state.active_modifiers if item.kind is ModifierKind.NEXT_ATTACK_MADE_DISADVANTAGE)


def consume_next_attack_made_disadvantage(state: CombatantState) -> int:
    before = len(state.active_modifiers)
    state.active_modifiers = [
        item for item in state.active_modifiers
        if not (item.kind is ModifierKind.NEXT_ATTACK_MADE_DISADVANTAGE and item.consume_on_attack_made)
    ]
    return before - len(state.active_modifiers)
