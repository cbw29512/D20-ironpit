...
def resolve_healing(
    ...
    rolls = [action.dice_size for _ in range(action.dice_count)] if healing_is_maximized(target.state, healer.state, action.id) else [dice.roll(action.dice_size) for _ in range(action.dice_count)]
