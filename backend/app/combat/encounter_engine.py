...
from app.combat.start_turn_save_auras import resolve_start_turn_save_auras
...
def _resolve_zero_hp_turn(sequence, round_number, combatant, dice) -> tuple[BattleEvent | None, int]:
    state = combatant.state
    if state.template.kind != "character" or state.current_hp != 0 or state.is_dead or state.is_stable:
        return None, sequence
    return resolve_death_save(sequence, round_number, combatant.combatant_id, state, dice), sequence + 1


def _flush_deaths(events, sequence, round_number, setup, dice):
...
def _end_turn_lifecycle(sequence, round_number, member, setup, dice):
    ...
    target_events, sequence = resolve_target_condition_timing(sequence, round_number, member, "target_turn_end", dice)
    events.extend(target_events)
    source_events, sequence = resolve_source_condition_timing(sequence, round_number, member, setup, "source_turn_end")
    events.extend(source_events)
    events, sequence = _flush_deaths(events, sequence, round_number, setup, dice)
    expire_source_turn_modifiers(
        [entry.state for entry in [*setup.heroes, *setup.monsters]], member.combatant_id, round_number,
    )
    return events, sequence


def run_encounter(selection: EncounterSelection, dice: DiceProvider) -> EncounterBattleResult:
    """Run the certified combat subset over a 1-6 vs. 1-6 encounter."""
    try:
        ...
        for round_number in range(1, MAX_ENCOUNTER_ROUNDS + 1):
            for combatant_id in initiative.turn_order:
                ...
                events.extend(more)
                events, sequence = _flush_deaths(events, sequence, round_number, setup, dice)
                if member.state.is_dead: continue
                more, sequence = resolve_start_turn_recharges(sequence, round_number, member, dice)
                events.extend(more)
                end_concentration_if_expired(member.state, round_number, affected_states)
                more, sequence = expire_start_of_turn_conditions(sequence, round_number, member, setup)
                events.extend(more)
                more, sequence = resolve_target_condition_timing(sequence, round_number, member, "target_turn_start", dice)
                events.extend(more)
                more, sequence = resolve_start_turn_save_auras(sequence, round_number, member, setup, dice)
                events.extend(more)
                events, sequence = _flush_deaths(events, sequence, round_number, setup, dice)
                ...
                if member.state.current_hp <= 0 or member.state.is_dead:
                    more, sequence = _end_turn_lifecycle(sequence, round_number, member, setup, dice)
                    events.extend(more); continue
                target = select_nearest_target(member, setup)
                if target is not None:
                    more, sequence = resolve_combat_turn(sequence, round_number, member, target, setup, dice)
                    events.extend(more)
                more, sequence = _end_turn_lifecycle(sequence, round_number, member, setup, dice)
                events.extend(more)
            outcome = resolve_encounter_outcome(setup)
