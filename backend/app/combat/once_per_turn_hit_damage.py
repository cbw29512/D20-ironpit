...
def once_per_turn_weapon_hit_bonus_damage(
    ...
    """Consume one declared weapon-hit rider for the active turn."""
    try:
        ...
        if turn_key is None:
            raise ValueError(
                f"{rider.source_name} requires the active-turn key for once-per-turn tracking."
            )
        ...
        logger.exception(
            "Failed once-per-turn weapon-hit rider resolution for %s using %s.",
