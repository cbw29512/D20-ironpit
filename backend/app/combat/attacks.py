...
def resolve_attack(
    ...
    attack: WeaponAttack, distance_ft: int, dice: DiceProvider, actor_event_id: str | None = None,
    target_event_id: str | None = None, spend_action: bool = True, advantage_sources: int = 0,
    other_disadvantage_sources: int = 0, feature_id: str | None = None, turn_key: str | None = None,
    bonus_damage: BonusDamageSpec | None = None, close_enemy_active: bool = True,
    redirect_target: CombatantState | None = None, redirect_target_event_id: str | None = None,
    affected_states: list[CombatantState] | None = None, sneak_attack_ally_available: bool = False,
    off_turn: bool = False, saving_throw_advantage_sources: int = 0,
) -> BattleEvent:
    try:
        if spend_action and not is_available(attacker, "action"): raise ValueError("Action is not available for an attack.")
        if attack.on_hit_saving_throw is not None and attack.weapon.mastery_property == "topple": raise ValueError("One attack event cannot audit both Topple and an on-hit saving throw.")
        ...
        if redirect_target is not None and redirect_target is not defender and defender.template.redirect_attack_reaction is not None and is_available(defender, "reaction"):
            spend(defender, "reaction"); actual_defender = redirect_target; actual_event_id = redirect_target_event_id or redirect_target.template.id; redirect_used = True
        natural = attack_roll.selected_roll or 0; natural_20 = natural == 20; natural_1 = natural == 1; natural_1_ends_turn = natural_1 and not off_turn
        ...
        hit_save = AttackSaveRiderOutcome(); weapon_sap_applied = False; tactical_sap_applied = False; vex_applied = False; studied_applied = False
        if hit:
            active_turn_key = turn_key or f"{round_number}:{attacker_event_id}"
            damage_roll, rolled_components = resolve_weapon_damage(attacker, attack, dice, critical, mode, active_turn_key, bonus_damage=bonus_damage, target=actual_defender, sneak_attack_ally_available=sneak_attack_ally_available)
            ...
            hit_save = resolve_attack_save_rider(attacker, actual_defender, attack, dice, round_number=round_number, attacker_event_id=attacker_event_id, distance_ft=distance_ft, affected_states=affected_states, advantage_sources=saving_throw_advantage_sources)
            ...
            vex_applied = apply_vex_mastery(attacker, attacker_event_id, actual_event_id, attack, round_number, applied_total); end_rage_if_incapacitated(actual_defender)
        else:
