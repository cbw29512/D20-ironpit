...
def resolve_attack(
    ...
    try:
        ...
        hit, miss_override_feature_id = apply_miss_to_hit_override(attacker, hit=hit)
        ...
        if miss_override_feature_id is not None: description += f" {miss_override_feature_id.replace('-', ' ').title()} turns the miss into a hit."
