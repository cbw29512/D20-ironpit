...
def resolve_rampage(
    ...
    """Resolve the reusable 2014 Rampage trigger through shared movement and attack rules."""
    try:
        ...
        if not is_available(attacker.state, "bonus_action") or not _melee_zero_hp_triggered(prior_events, attacker):
            return [], sequence
        bite = next((attack for attack in [attacker.state.template.weapon_attack, *attacker.state.template.alternate_weapon_attacks] if attack.id == "bite"), None)
        if bite is None:
        ...
        if choice is None:
            ...
            if not targets:
                return [], sequence
            target = min(targets, key=lambda item: abs(item.position_ft - attacker.position_ft))
