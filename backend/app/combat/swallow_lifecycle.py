...
def _turn_damage(events: list[BattleEvent], attacker_id: str, target_id: str) -> int:
    total = 0
    for event in events:
        if event.actor_id != attacker_id or event.target_id != target_id:
            continue
        if event.hp_before is None or event.hp_after is None:
            continue
        total += max(0, event.hp_before - event.hp_after)
    return total
...
def _grid_release_position(
    ...
    arena = setup.map_definition
    origin = source.state.position
...
def resolve_end_turn_regurgitation(
    ...
    roll, succeeded = resolve_saving_throw(
        source.state, swallowed.regurgitation_save_ability, swallowed.regurgitation_save_dc, dice,
    )
    released = []
    if not succeeded:
        for victim in list(_members(setup)):
            ...
            if state is None or state.source_id != source.combatant_id:
                continue
            _release(victim, source, setup, swallowed.regurgitation_range_ft)
    ...
    event = BattleEvent(
        ...
        save_ability=swallowed.regurgitation_save_ability, save_dc=swallowed.regurgitation_save_dc,
        save_succeeded=succeeded, animation="forced-movement", description=description,
    )
