...
def resolve_pending_death_triggers(
    ...
    while True:
        ...
        for source, action in pending:
            ...
            shared = [
                dice.roll(action.damage_dice_size) for _ in range(action.damage_dice_count)
            ] if action.damage_dice_count else None
            for target in _targets(source, action, setup):
                event = resolve_save_action(
                    sequence, round_number, source, target, action,
                    combatant_distance(source, target), dice,
