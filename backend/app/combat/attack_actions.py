...
from app.combat.attack_action_choices import area_save_choice, attack_choice, save_choice, slot_has_legal_choice, use_ranged_split
from app.combat.attack_action_policy import expanded_slots, filtered_slot, required_attack_id, required_target_id, slot_allowed
    ...
    try:
        ...
        for index, raw_slot in expanded_slots(definition, dice):
            ...
            if not slot_allowed(definition, index, previous_event):
                continue
            slot = filtered_slot(definition, raw_slot, used_attack_ids, required_attack_id(definition, index, previous_attack_id))
            split_this_slot = index > 0 and ranged_split and not ranged_split_used and flexible_slot_has_both(attacker, slot.attack_ids)
            chosen_attack = None
            if attack_limit is None or attacks_made < attack_limit:
            ...
            if chosen_area is not None:
                ...
                events.extend(area_events); previous_event = area_events[-1] if area_events else previous_event
                sequence = append_pending_death_triggers(events, sequence, round_number, setup, dice)
                continue
            chosen_save = save_choice(attacker, setup, slot)
            if chosen_save is not None:
                target, save_action, distance = chosen_save
                event = resolve_save_action(sequence, round_number, attacker, target, save_action, distance, dice,
