...
from app.combat.pit_policy import (
    allied_frontline_active, choose_attack, flexible_slot_has_both, has_backline_target,
    has_frontline_target, is_backline, save_distance, target_order,
)
...
def _attack_choice(attacker, setup, slot, *, ranged_backline: bool = False):
    ...
    if is_backline(attacker) and allied_frontline_active(attacker, setup):
        ...
        if ranged is not None: return ranged
    melee = choose_attack(attacker, setup, slot.attack_ids, kind=WeaponAttackKind.MELEE)
    return melee if melee is not None else choose_attack(attacker, setup, slot.attack_ids, kind=WeaponAttackKind.RANGED)


def _use_ranged_split(attacker, setup, slots, dice: DiceProvider) -> bool:
    if is_backline(attacker) or not has_frontline_target(attacker, setup) or not has_backline_target(attacker, setup): return False
    if not any(flexible_slot_has_both(attacker, slot.attack_ids) for slot in slots[1:]): return False
...
def _flush(events, sequence, round_number, setup, dice):
    ...
    events.extend(triggered); return sequence
...
def resolve_attack_action(
    ...
    """Resolve Multiattack/Extra Attack, interrupting between attacks for death triggers."""
    try:
        validate_attack_action_slots(attacker)
        definition = attacker.state.template.attack_action
        if definition is None or not is_available(attacker.state, "action"): raise ValueError("Attack action or Multiattack is not available.")
        if not target_order(attacker, setup): return [], sequence
        ...
        ranged_split = _use_ranged_split(attacker, setup, definition.slots, dice); ranged_split_used = False
        ...
        for index, slot in enumerate(definition.slots):
            ...
            if attack_choice is not None:
                target, attack, distance = attack_choice; spend_attack_resource(attacker.state, attack)
                if split_this_slot and attack.weapon.attack_kind is WeaponAttackKind.RANGED: ranged_split_used = True
                ...
                event = resolve_encounter_attack(
                    ...
                    turn_key=turn_key, allow_reckless=True, close_enemy_active=False,
                )
                ...
                events.extend(cleave); sequence = _flush(events, sequence, round_number, setup, dice)
                if definition.is_attack_action and light_trigger is None and attack.weapon.light: light_trigger = attack
                opening_feature = None; continue
            save_choice = _save_choice(attacker, setup, slot)
            if save_choice is not None:
                target, save_action, distance = save_choice
                events.append(resolve_save_action(sequence, round_number, attacker, target, save_action, distance, dice, spend_action=False, affected_states=affected_states))
                sequence += 1; sequence = _flush(events, sequence, round_number, setup, dice)
        if definition.is_attack_action and light_trigger is not None and not attacker.state.is_dead and not attacker.state.is_unconscious:
            more, sequence = resolve_light_extra_attack(sequence, round_number, attacker, setup, dice, light_trigger, turn_key)
            events.extend(more); sequence = _flush(events, sequence, round_number, setup, dice)
