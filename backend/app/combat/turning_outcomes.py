...
def challenge_rating_at_or_below(
    ...
    """Compare printed Challenge Rating with a source-owned turning threshold."""
    try:
        ...
        raise ValueError(
            f"Invalid turning-destruction CR comparison for {target.state.template.name}."
        ) from exc
...
def destroy_on_failed_turn_save_if_eligible(
    ...
    *,
    ...
    """Resolve a generic failed-turn-save zero-HP threshold without damage semantics."""
    try:
        ...
        logger.exception("Failed turning destruction outcome for %s.", target.state.template.name)
