...
def arm_deferred_save_effect(
    ...
    """Arm one configured deferred effect after a qualifying hit."""
    try:
        ...
        if resource is None or resource.current_uses < rule.resource_cost:
            return None
        if any(item.source_id == rule.source_id for item in attacker.deferred_effects):
            return None
        resource.current_uses -= rule.resource_cost
        attacker.deferred_effects.append(DeferredEffectState(source_id=rule.source_id, target_id=target_id))
...
def deferred_save_effect_candidate(
    ...
    """Return the currently marked active target if the deferred Action is legal."""
    ...
    if rule is None or not is_available(actor.state, "action"):
        return None
    mark = next((item for item in actor.state.deferred_effects if item.source_id == rule.source_id), None)
    if mark is None:
        return None
    ...
    if target is None or target.state.is_dead or not target.state.is_alive or target.state.current_hp <= 0:
        actor.state.deferred_effects = [item for item in actor.state.deferred_effects if item.source_id != rule.source_id]
        return None
    return target
...
def resolve_deferred_save_effect(
    ...
    try:
        ...
        if succeeded:
            rolls = [dice.roll(rule.success_damage_dice_size) for _ in range(rule.success_damage_dice_count)]
            total = sum(rolls)
            ...
            apply_damage(target.state, applied, damage_types={damage_type}, dice=dice, affected_states=affected)
        ...
        elif rule.failure_sets_zero_hp:
            reduce_to_zero_hit_points(target.state, dice=dice, affected_states=affected)
        spend(actor.state, "action")
        actor.state.deferred_effects = [item for item in actor.state.deferred_effects if item.source_id != rule.source_id]
        description = (
            f"{actor.state.template.name} activates {rule.source_id.replace('-', ' ').title()} on "
            f"{target.state.template.name}; the Constitution save {'succeeds' if succeeded else 'fails'}."
        )
        return BattleEvent(
            sequence=sequence,
