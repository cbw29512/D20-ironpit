...
def arm_deferred_save_effect(
    ...
    try:
        ...
        return rule.source_id, rule.source_name
    except ValueError:
        raise
...
def deferred_save_effect_candidate(
    ...
    """Return a legal marked target without mutating discovery state."""
    ...
    try:
        ...
        if succeeded and rule.success_damage_dice_count:
            if not rule.success_damage_type:
                raise ValueError(
                    f"Deferred effect {rule.source_id} has success damage dice without a damage type."
                )
            rolls = [
