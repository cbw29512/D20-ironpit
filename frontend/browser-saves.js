(() => {
  ...
  function resolveAction(sequence, round, actor, target, action, distance, options = {}) {
    ...
    const effectTags = new Set((action.effectTags || []).map((tag) => String(tag).toLowerCase()));
    if (action.damageType === "poison") effectTags.add("poison");
    const saveContext = { magicalEffect: Boolean(action.magicalEffect), effectTags: [...effectTags] };
