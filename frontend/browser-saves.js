(() => {
  ...
  function resolveSavingThrow(state, ability, dc) {
    if (state.is_unconscious && (ability === "strength" || ability === "dexterity")) return { roll: null, succeeded: false };
  ...
  function resolveAction(sequence, round, actor, target, action, distance, options = {}) {
    ...
    if (count && !(save.succeeded && action.successDamage === "none")) {
      ...
      if (applied) {
        A().applyDamage(target.state, applied, false);
    ...
    return {
      sequence, round_number: round, event_type: "saving_throw", actor_id: actor.combatant_id,
      actor_name: actor.state.template.name, target_id: target.combatant_id, target_name: target.state.template.name,
      saving_throw_roll: save.roll, save_ability: action.saveAbility, save_dc: action.dc, save_succeeded: save.succeeded,
