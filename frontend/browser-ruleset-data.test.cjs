...
assert.equal(heroes2014.length, 70, "2014 browser heroes must contain Fighter and Barbarian 1-20, and Rogue, Monk, Paladin 1-10");
...
const barbarian14 = barbarians2014.find((hero) => hero.level === 14);
const barbarian15 = barbarians2014.find((hero) => hero.level === 15);
const barbarian17 = barbarians2014.find((hero) => hero.level === 17);
const barbarian18 = barbarians2014.find((hero) => hero.level === 18);
const barbarian20 = barbarians2014.find((hero) => hero.level === 20);
assert.deepEqual(barbarian14.damage_reaction_attack, {
  source_feature: "retaliation", trigger: "damaged-by-creature", source_range_ft: 5, attack_kind: "melee",
});
assert.equal(barbarian15.persistent_rage_2014, true);
assert.equal(barbarian17.brutal_critical_dice, 3);
assert.equal(barbarian17.resources.rage, 6);
assert.equal(barbarian18.ability_check_minimums[0].source_id, "indomitable-might");
assert.deepEqual([barbarian20.ability_scores.strength, barbarian20.ability_scores.constitution], [24, 24]);
assert.equal(Object.hasOwn(barbarian20.resources, "rage"), false);
assert.deepEqual(barbarian20.unlimited_resources, ["rage"]);
