(() => {
  ...
  function crNumber(value) {
    const text = String(value || "0"); if (!text.includes("/")) return Number(text) || 0;
  ...
  function totalCr(values) {
    ...
    const divisor = quarters % 2 === 0 ? 2 : 4; return `${quarters / (4 / divisor)}/${divisor}`;
  }
  function defeatedMember(member) {
    const state = member.state;
    if (state.template.kind === "character") return state.is_dead || !state.is_alive;
    const deferred = state.current_hp <= 0 && state.is_alive && !state.is_dead
      && (state.template.regeneration || []).some((effect) => effect.requiresPositiveHp === false);
    return deferred ? false : state.current_hp <= 0 || state.is_dead || !state.is_alive;
  }
  ...
  function runEncounter(selection) {
    ...
    for (let round = 1; round <= 100; round += 1) {
      ...
      for (const id of init.turn_order) {
        ...
        const member = byId.get(id); B()?.cleanupDisabledSources(setup);
