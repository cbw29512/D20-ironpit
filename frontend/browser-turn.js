(() => {
  ...
  function resolveTurn(sequence, round, member, setup) {
    ...
    for (const item of recharge) { const rule = member.state.template.resource_recharge?.[item.id] || {}; const outcome = item.restored ? "recharges" : "does not recharge"; events.push({ sequence: sequence++, round_number: round, event_type: "feature", actor_id: member.combatant_id, actor_name: member.state.template.name, feature_id: `recharge:${item.id}`, attack_roll: { notation: "1d6", rolls: [item.roll], selected_roll: item.roll, modifier: 0, mode: "normal", total: item.roll }, resource_remaining: member.state.resources[item.id], animation: "resource", description: `${member.state.template.name} rolls ${item.roll} for ${rule.name || item.id} (Recharge ${rule.minimum}–6) and ${outcome}.`, audit: { schema_version: 1, steps: [{ phase: "roll", kind: "roll", label: `Recharge roll: ${item.roll}` }, { phase: "resource_change", kind: "resource", label: `${rule.name || item.id}: ${member.state.resources[item.id]}/${rule.maxUses || 1}` }] } }); }
