(() => {
  ...
  const D = () => window.IRON_PIT_DICE, R = () => window.IRON_PIT_BROWSER_ROLLS;
  const X = () => window.IRON_PIT_BROWSER_MODIFIERS || { applyD20Bonus: (_state, _kind, roll) => roll };
  ...
  function deathSave(sequence, round, member, advantageSources = 0) {
    const state = member.state, roll = X().applyD20Bonus(state, "saving-throw-bonus-die", R().d20(0, R().modeFromSources(advantageSources, 0))), natural = roll.selected_roll;
    const successesBefore = state.death_save_successes, failuresBefore = state.death_save_failures; let result = "failure";
    ...
    else if (roll.total >= 10) { state.death_save_successes = Math.min(3, state.death_save_successes + 1); result = "success"; } else state.death_save_failures = Math.min(3, state.death_save_failures + 1);
    ...
    return { sequence, round_number: round, event_type: "death_save", actor_id: member.combatant_id, actor_name: state.template.name,
      death_save_roll: roll, hp_after: state.current_hp,
