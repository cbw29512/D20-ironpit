(() => {
  ...
  function effectiveSpeed(state) {
    const preventsMagicalReduction = (state.timed_effects || []).some((effect) => effect.prevents_magical_speed_reduction);
    const modifierTotal = (state.active_modifiers || [])
      .filter((item) => item.kind === "speed")
      .filter((item) => !(preventsMagicalReduction && item.source_is_magical && (item.flat_bonus || 0) < 0))
      .reduce((sum, item) => sum + (item.flat_bonus || 0), 0);
    const speed = Math.max(0, state.template.speed_ft + modifierTotal);
    return X()?.effectiveSpeed(state, speed) ?? speed;
  }
