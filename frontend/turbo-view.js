(() => {
  ...
  function render(batch) {
    try {
      ...
      const input = el("turbo-fight-number"); input.max = String(batch.requested_fights); input.value = String(batch.fastest_fight_number || 1);
      el("turbo-replay-button").disabled = batch.valid_fights === 0;
      const errorBox = el("turbo-error-summary"); errorBox.hidden = errors === 0;
      errorBox.textContent = errors ? `${errors} fight${errors === 1 ? "" : "s"} hit an engine-rule error and were excluded from the ratios.` : "";
      panel.hidden = false;
