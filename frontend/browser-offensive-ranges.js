(() => {
  ...
  function rechargePriority(member, resourceId) {
    try {
      ...
      console.error("Failed browser Recharge movement priority probe", { member: member.combatant_id, resourceId, error }); throw error;
    }
  }
  ...
  function effectiveActionRange(action) {
    try {
      ...
      console.error("Failed browser action effective range probe", { action: action.id, error }); throw error;
    }
  }
  function spellResourceAvailable(member, action, turnKey) {
    try { return C().actionResourceAvailable(member.state, action, turnKey); }
    catch (error) { console.error("Failed browser offensive spell resource probe", { member: member.combatant_id, action: action.id, error }); throw error; }
  }
  function saveTargetEligible(member, action, target) {
    ...
    if (action.requiredTargetGrappledBySelf && !(target.state.grapple_sources || []).some((source) => source.source_id === member.combatant_id)) return false;
    ...
    try {
      ...
      for (const attack of member.state.template.attacks || []) {
        ...
        if (attack.kind === "melee" || attack.kind === "melee_or_ranged") {
          ...
          ranges.push({ family: "melee", range: reach, maxRange: reach, preferredRange: reach, priority, executionRank, expectedValue });
        }
        if (attack.kind === "ranged" || attack.kind === "melee_or_ranged") {
          ...
          if (!Number.isFinite(maximum) || !Number.isFinite(preferred)) continue;
          const expectedValue = setup && O()?.weaponAttack ? O().weaponAttack(member, target, attack, setup, preferred) : 0;
          ranges.push({ family: "ranged", range: maximum, maxRange: maximum, preferredRange: preferred, priority, executionRank, expectedValue });
        }
      }
      return ranges;
    } catch (error) { console.error("Failed browser weapon-range probe", { member: member.combatant_id, error }); throw error; }
  }
  function saveActionRanges(member, target) {
    try {
      ...
      for (const action of member.state.template.saving_throw_actions || []) {
        ...
        const priority = rechargePriority(member, action.resourceId), distance = effectiveActionRange(action);
        const expectedValue = O()?.saveAction?.(target, action) || 0;
        const executionRank = priority === 0 ? 0 : (saveIds.has(action.id) ? 2 : 3);
        ranges.push({ family: "ability", range: distance, maxRange: distance, preferredRange: distance, priority, executionRank, expectedValue });
      }
      return ranges;
    } catch (error) { console.error("Failed browser save-action range probe", { member: member.combatant_id, error }); throw error; }
  }
  function spellRanges(member, target, setup, turnKey) {
    try {
      const ranges = [];
      const add = (distance, expectedValue = 0) => ranges.push({ family: "spell", range: distance, maxRange: distance, preferredRange: distance, priority: 1, executionRank: 1, expectedValue });
      for (const action of member.state.template.spell_attack_actions || []) {
        ...
        if (spellResourceAvailable(member, action, turnKey)) add(action.range || 0, target && setup && O() ? O().spellAttack(member, target, action, setup) : 0);
      }
      for (const action of member.state.template.spell_save_actions || []) {
        ...
        if (spellResourceAvailable(member, action, turnKey)) add(effectiveActionRange(action), target && O() ? O().saveSpell(target, action) : 0);
      }
      for (const action of member.state.template.automatic_spell_actions || []) {
        if (action.actionCost === "reaction" || !E().available(member.state, action.actionCost)) continue;
        if (spellResourceAvailable(member, action, turnKey)) add(action.range || 0, target && O() ? O().automaticSpell(target, action) : 0);
      }
      return ranges;
    } catch (error) { console.error("Failed browser spell-range probe", { member: member.combatant_id, error }); throw error; }
  }
  function rangesForTarget(member, target, turnKey, setup = null) {
    try { return [...weaponRanges(member, target, setup), ...spellRanges(member, target, setup, turnKey), ...saveActionRanges(member, target)]; }
    catch (error) { console.error("Failed browser offensive-range inventory", { member: member.combatant_id, target: target.combatant_id, error }); throw error; }
  }
