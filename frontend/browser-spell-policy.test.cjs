...
const member = (id, side, position, template = base) => {
  ...
  state.position = { x: Math.floor(position / 5), y: 0 };
  return { combatant_id: id, side, position_ft: position, state };
};
...
const setup = (c, monsters, allies = []) => ({
  heroes: [c, ...allies], monsters,
  map_definition: { id: "spell-policy-test", width_squares: 24, height_squares: 16 },
});

{
  ...
  const choice = P.choose(c, setup(c, monsters), "1:caster");
...

{
  ...
  const choice = P.choose(c, setup(c, monsters, [ally]), "1:caster");
  assert.equal(choice.action.id, "fireball");
  assert.deepEqual(new Set(choice.targetIds), new Set(["monster-0", "monster-1"]));
}

{
  ...
  const monsters = [member("monster-0", "monsters", 5), member("monster-1", "monsters", 5)];
  const choice = P.choose(c, setup(c, monsters, [ally]), "1:caster");
  assert.equal(choice.action.id, "burst");
  assert.deepEqual(new Set(choice.targetIds), new Set(["monster-0", "monster-1"]));
}

{
  ...
  const encounter = setup(c, monsters);
  const choice = P.choose(c, encounter, "1:caster");
  window.IRON_PIT_DICE = queuedDice([1, 6, 1, 6, 1, 6]);
  const result = X.resolve(1, 1, c, encounter, choice, "1:caster");
  assert.equal(result.events.length, 4);
  assert.match(result.events[0].description, /Area covers 3 enemies\./);
