(() => {
  ...
  function install() {
    ...
    register({
      ...
      discover: ({ member, setup }) => {
        const runtime = DS();
        if (!runtime && member.state.template.deferred_save_effect) {
          throw new Error("Deferred save effect runtime is not loaded.");
        }
        const target = runtime?.candidate(member, setup) || null;
