(() => {
  ...
  function active(state, attack) {
    try {
      if (!state?.template?.traits?.includes("bloodied-fury")) return false;
      if (attack?.kind !== "melee") return false;
      const maximum = S()?.effectiveMaxHp?.(state);
      if (!Number.isFinite(maximum) || maximum <= 0) {
        throw new Error("Bloodied Fury requires an effective maximum HP value.");
      }
      return state.current_hp * 2 <= maximum;
    } catch (error) {
      console.error("Browser Bloodied Fury eligibility failed", {
        combatant: state?.template?.id || state?.template?.name,
        attack: attack?.id,
        error,
      });
      throw error;
  ...
  window.IRON_PIT_BROWSER_BLOODIED_FURY = {
    active, installAbilityHooks, resolveBeforeAttackRoll,
  };
