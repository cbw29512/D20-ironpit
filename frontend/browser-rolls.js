(() => {
  ...
  function candidate(attack, critical) {
    const count = attack.diceCount * (critical ? 2 : 1), rawRolls = dice().rollMany(count, attack.diceSize);
    const rolls = attack.damageDieMinimum ? rawRolls.map((roll) => Math.max(roll, attack.damageDieMinimum)) : rawRolls;
    return { notation: `${count}d${attack.diceSize}+${attack.damageBonus}`, rolls, modifier: attack.damageBonus,
      total: rolls.reduce((sum, roll) => sum + roll, 0) + attack.damageBonus };
  }
  const fixedCandidate = (attack) => ({ notation: String(attack.fixedDamage), rolls: [], modifier: 0, total: attack.fixedDamage });
  function damageComponent(spec, critical) {
    const modifier = spec.damageBonus || 0;
    if (spec.diceCount === 0) return { source: spec.source, damage_type: spec.damageType, notation: String(modifier), rolls: [], modifier: 0, total: modifier };
    const count = spec.diceCount * (critical ? 2 : 1), rolls = dice().rollMany(count, spec.diceSize);
    return { source: spec.source, damage_type: spec.damageType, notation: `${count}d${spec.diceSize}+${modifier}`,
      rolls, modifier, total: rolls.reduce((sum, roll) => sum + roll, 0) + modifier };
  }
  const bonusComponent = (spec, critical) => damageComponent({ ...spec, damageBonus: 0 }, critical);
  ...
  function savageRevision(first, second, useSecond) {
    return { source_effect_id: "savage-attacker", kind: "roll_twice_choose",
      ...
      original_selected: null, replacement_selected: null, original_total: first.total,
      replacement_total: second.total, accepted: useSecond ? "replacement" : "original", replaced_die_index: null };
  }
  function addExtraCriticalDice(components, attacker, attack, critical, replacement) {
    const count = critical && !replacement && attack.fixedDamage == null && attack.kind === "melee"
    ...
    components.push({ source: "Extra critical weapon dice", damage_type: attack.damageType,
      notation: `${count}d${attack.diceSize}+0`, rolls, modifier: 0, total: rolls.reduce((sum, value) => sum + value, 0) });
  }
  function weaponDamage(attacker, attack, critical, mode, turnKey, bonusDamage = null, target = null, sneakAllyAvailable = false) {
    ...
    const replacement = conditional?.mode === "replace_weapon" && conditionalActive(conditional, attacker, target, mode) ? conditional : null;
    let rolled;
    if (replacement) rolled = candidate(replacement, critical);
      ...
      if (attacker.template.traits?.includes("savage-attacker") && attacker.feature_last_turn_keys["savage-attacker"] !== turnKey) {
        ...
        rolled = useSecond ? second : first; rolled.revisions = [...(rolled.revisions || []), savageRevision(first, second, useSecond)];
    ...
    addExtraCriticalDice(components, attacker, attack, critical, replacement);
    ...
    if (mode === "advantage" && attack.conditionalAdvantage) {
      const [baseCount, sides] = attack.conditionalAdvantage, count = baseCount * (critical ? 2 : 1), rolls = dice().rollMany(count, sides);
    ...
    return { roll: { notation: components.map((item) => item.notation).join(" + "),
      rolls: components.flatMap((item) => item.rolls), modifier: components.reduce((sum, item) => sum + item.modifier, 0), total }, components };
  }
