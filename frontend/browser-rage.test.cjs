...
function testBarbarian() {
  return {
    id: "test-barbarian-l1", name: "Test Barbarian", archetype: "Barbarian", level: 1,
    kind: "character", size: "medium", armor_class: 14, max_hp: 15, speed_ft: 30,
    initiative_bonus: 2,
    attacks: [{
      id: "test-greataxe", name: "Greataxe", kind: "melee", bonus: 5,
      diceCount: 1, diceSize: 12, damageBonus: 3, damageType: "slashing",
      reach: 5, animation: "heavy-slash", rageEligible: true,
    }],
    primary_attack_id: "test-greataxe", traits: [], resources: { rage: 2 },
    rage_damage_bonus: 2, wearing_heavy_armor: false,
  };
}

{
  const barbarian = testBarbarian();
  window.IRON_PIT_BROWSER_HEROES[barbarian.id] = barbarian;
  window.IRON_PIT_DICE = queuedDice([20, 1, 15, 6]);
  const battle = window.IRON_PIT_BROWSER_ENGINE.runEncounter({
    hero_ids: [barbarian.id], monster_ids: ["srd-commoner"], starting_distance_ft: 5,
  });
  ...
  assert.ok(rage, "expected Rage to activate before attacking");
  assert.ok(attack?.hit, "expected deterministic Barbarian hit");
  ...
  delete window.IRON_PIT_BROWSER_HEROES[barbarian.id];
}

{
  const barbarian = testBarbarian();
  const bandit = structuredClone(window.IRON_PIT_BROWSER_MONSTERS["srd-bandit"]);
  const hero = { combatant_id: "hero-1:test", side: "heroes", position_ft: 0, state: window.IRON_PIT_BROWSER_STATE.buildState(barbarian) };
  ...
  assert.equal(hero.state.current_hp, 12);
