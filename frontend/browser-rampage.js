(() => {
  ...
  function triggered(events, member) {
    try {
      const meleeIds = new Set(
        (member.state.template.attacks || [])
          .filter((attack) => attack.kind === "melee")
          .map((attack) => attack.id),
      );
      return events.some((event) => event.event_type === "attack"
        && event.actor_id === member.combatant_id
        && meleeIds.has(event.weapon_id)
        && (event.hp_before || 0) > 0
        && (event.hp_after || 0) <= 0);
    } catch (error) {
      console.error("Failed browser Rampage trigger check", { member: member?.combatant_id, error });
      throw error;
    }
  }

  function biteAttack(member) {
    const bite = (member.state.template.attacks || []).find(
      (attack) => attack.kind === "melee" && String(attack.name).toLowerCase() === "bite",
    );
  ...
  function resolve(sequence, round, member, setup, priorEvents, turnKey) {
    try {
      if (!member.state.template.traits?.includes("rampage")
        || !E().available(member.state, "bonus_action")
        || !triggered(priorEvents, member)) {
        return { events: [], sequence };
      }

      const bite = biteAttack(member);
      ...
      if (!choice) {
        ...
        const target = targets.reduce(
          (best, item) => S().distance(member, item) < S().distance(member, best) ? item : best,
        );
        ...
        const moved = R().moveToward(
          sequence, round, member, target, setup, bite.reach || 5, "speed", { turnKey },
        );
