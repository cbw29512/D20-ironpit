(() => {
  ...
  function markRecklessUse(state, turnKey) {
    if (state.template.ruleset !== "2024" || !turnKey || !state.template.frenzy || !R()?.active(state)) return;
    state.feature_last_turn_keys[RECKLESS_MARKER] = turnKey;
  }

  function bonusDamage(state, attack, turnKey) {
    if (state.template.ruleset !== "2024" || !turnKey || !state.template.frenzy || !R()?.active(state) || !B2()?.active(state)) return null;
