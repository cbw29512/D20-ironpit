(() => {
  ...
  function apply(state, effectId, sourceId, options = {}) {
    ...
    state.timed_effects.push({
      effect_id: effectId, source_id: sourceId, source_effect_id: sourceEffectId,
      applied_round: options.appliedRound || null, expires_round: options.expiresRound || null,
      expires_at_start_of_source_turn: arenaPoison ? false : expiryTiming === "source_turn_start", expiry_timing: expiryTiming,
      ...
      allowed_removal_action_ids: [...(options.allowedRemovalActionIds || [])], turn_behavior: options.turnBehavior || "normal",
      ends_on_damage: Boolean(options.endsOnDamage), ends_if_source_incapacitated: Boolean(options.endsIfSourceIncapacitated),
      ends_if_source_dead: Boolean(options.endsIfSourceDead), source_effect_immunity_on_end: Boolean(options.sourceEffectImmunityOnEnd),
  ...
  function removeGroup(state, effect) {
    ...
    const grouped = state.timed_effects.filter((item) => item.source_id === effect.source_id && item.source_effect_id === effect.source_effect_id);
    const removed = [];
  ...
  function expireSourceStart(sequence, round, source, setup) {
    ...
    for (const target of [...setup.heroes, ...setup.monsters]) {
      const expiring = target.state.timed_effects.filter((effect) => effect.source_id === source.combatant_id
      ...
      for (const effect of expiring) {
        ...
        events.push({ sequence: sequence++, round_number: round, event_type: "feature", actor_id: source.combatant_id,
          actor_name: source.state.template.name, target_id: target.combatant_id, target_name: target.state.template.name,
          removed_condition_ids: removed, feature_id: effect.source_effect_id || "condition-ended", animation: "condition-ended",
          description: `${target.state.template.name} is no longer affected by ${effect.source_effect_id || effect.effect_id}.` });
      }
    }
