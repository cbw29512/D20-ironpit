...
window.IRON_PIT_BROWSER_ATTACK = {
  adjustedDamage: (state, amount, type) =>
    state.fire_resistant && type === "fire" ? Math.floor(amount / 2) : amount,
...
window.IRON_PIT_DICE = {
  rollMany: () => {
    if (damageRollCalls >= damageRolls.length) {
      throw new Error("Split spell damage rolled more than once per component.");
    }
    return [...damageRolls[damageRollCalls++]];
  },
};
...
const member = (id, side, position, fireResistant = false) => ({
  combatant_id: id,
  side,
...
const targets = [
  member("target-1", "monsters", 30, true),
  member("target-2", "monsters", 30, false),
];
const choice = {
  action: {
    id: "split-flame",
    ...
    damageType: null,
    damageComponents: [
      { source: "Split Flame (Fire)", diceCount: 2, diceSize: 6, damageBonus: 0, damageType: "fire" },
      { source: "Split Flame (Radiant)", diceCount: 2, diceSize: 6, damageBonus: 0, damageType: "radiant" },
    ],
    successDamage: "half",
    upcastDicePerLevel: 0,
  },
  slotLevel: 0,
  targetIds: targets.map((target) => target.combatant_id),
  placement: {
    enemyIds: targets.map((target) => target.combatant_id),
    friendlyIds: [],
  },
};
...
assert.deepEqual(saves[0].damage_components[1].rolls, [3, 3]);
assert.deepEqual(saves[1].damage_components[1].rolls, [3, 3]);
assert.equal(saves[0].damage_components[0].applied_total, 4);
assert.equal(saves[0].damage_components[1].applied_total, 6);
assert.equal(saves[0].damage_roll.total, 10);
assert.equal(saves[1].damage_roll.total, 7);

console.log("Browser split save damage preserves typed defenses and shared AoE rolls.");
