(() => {
  ...
  function buildState(template) {
    return {
      ...
      is_unconscious: false, is_stable: false, is_dead: false, death_save_successes: 0, death_save_failures: 0,
      action_available: true, bonus_action_available: true, reaction_available: true, turn_terminated: false, turn_termination_reason: null,
      movement_remaining_ft: 0, resources: { ...(template.resources || {}) }, heroic_inspiration: false,
      active_effect_ids: [], active_buff_effect_ids: [], opening_buff_spell_id: null, grapple_sources: [], timed_effects: [], active_modifiers: [], concentration: null,
      feature_last_turn_keys: {}, spell_slot_expended_turn_key: null, temporary_damage_resistances: [], damage_types_since_last_turn: [], rage_expires_round: null, rage_max_round: null,
    };
  }
  ...
  function grantTemporaryHp(state, amount) {
    ...
    state.temporary_hp = Math.max(state.temporary_hp, amount); return state.temporary_hp;
  }

  function resolveRegeneration(state) {
    const damageTypes = new Set(state.damage_types_since_last_turn || []), results = [];
    const zeroHpRecovery = state.current_hp === 0 && (state.template.regeneration || []).some((effect) => effect.requiresPositiveHp === false);
    ...
    if (zeroHpRecovery && state.current_hp === 0) {
  ...
  function refreshStartOfTurn(state) {
    refreshReaction(state); window.IRON_PIT_BROWSER_HEROIC_INSPIRATION?.grant(state);
    resolveRegeneration(state); return U().refresh(state);
  }

  function beginTurn(state) {
    state.turn_terminated = false; state.turn_termination_reason = null;
    const incapacitated = Q().incapacitated(state); state.action_available = !incapacitated; state.bonus_action_available = !incapacitated;
    const rechargeResults = refreshStartOfTurn(state), speedZero = G()?.speedIsZero(state) || false, speed = M().effectiveSpeed(state);
  ...
  function targetPriority(member) {
    const state = member.state; if (!state.is_alive || state.is_dead) return null;
  ...
  function priorityTargets(member, setup) {
    ...
    const priority = Math.min(...eligible.map(targetPriority)); return eligible.filter((candidate) => targetPriority(candidate) === priority);
  }
  function nearestTarget(member, setup) {
    ...
    const grapplers = candidates.filter((candidate) => grapplerIds.has(candidate.combatant_id)), choices = grapplers.length ? grapplers : candidates;
  ...
  function moveToward(member, target, desired) {
    ...
    member.position_ft += (member.position_ft < target.position_ft ? 1 : -1) * moved; member.state.movement_remaining_ft -= moved;
