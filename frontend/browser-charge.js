(() => {
  ...
  function movementEvent(sequence, round, member, target, movement) {
    return { sequence, round_number: round, event_type: "movement",
  ...
  function resolveClosing(sequence, round, member, target) {
    const attack = member.state.template.attacks.find((item) => item.id === member.state.template.primary_attack_id)
    ...
    if (movementNeeded < profile.minimumMove || movementNeeded > member.state.movement_remaining_ft) return { events: [], sequence, handled: false };
    ...
    const attackEvent = A().resolveAttack(sequence++, round, member, target, attack, S().distance(member, target), {
      featureId: "charge",
      bonusDamage: { source: "Charge", diceCount: profile.diceCount, diceSize: profile.diceSize, damageType: profile.damageType },
