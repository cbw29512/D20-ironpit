(() => {
  ...
  function resolveClosing(sequence, round, member, target, setup = null) {
    ...
    if (needed < profile.minimumMove || needed > member.state.movement_remaining_ft || !S().canProne(target, profile.proneMaxSize)) {
      return { events: [], sequence, handled: false };
    }
    const moved = W().moveToward(sequence, round, member, target, setup, attack.reach || 5), events = [...moved.events];
    sequence = moved.sequence;
    if (!moved.movement) return { events, sequence, handled: events.length > 0 };
    if (moved.movement.moved < profile.minimumMove) return { events, sequence, handled: events.length > 0 };
