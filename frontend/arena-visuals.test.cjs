...
const fx = fs.readFileSync(path.join(root, "battlefield-fx.js"), "utf8");
...
assert.match(fx, /event\.event_type === "movement"/, "Movement events must drive card-local motion.");
assert.match(fx, /function ranged/, "Attack animation selection must be data-driven rather than monster-specific.");
assert.match(fx, /syncConditionPose/, "Universal conditions must drive persistent card poses.");
assert.match(replay, /IRON_PIT_BATTLEFIELD_FX/, "Replay must delegate presentation effects to the dedicated FX module.");
