(() => {
  "use strict";
  const MAX_SLOTS = 6;
  const state = { catalog: null, heroSlots: Array(MAX_SLOTS).fill(null), monsterSlots: Array(MAX_SLOTS).fill(null), fighting: false, hasRun: false, turboBatch: null };
  ...
  function invalidateRun() { state.hasRun = false; state.turboBatch = null; turboView()?.hide(); updateControls(); }
  ...
  async function fight() {
    ...
    try {
      ...
      const battle = window.IRON_PIT_BROWSER_ENGINE.runEncounter(match.selection);
      const rolls = dice().getHistory(), diagnosticId = lab().diagnosticId(match.selection.hero_ids, match.selection.monster_ids, rolls);
      await window.playIronPitBattle(battle, { heroes: match.heroes.indexes, monsters: match.monsters.indexes });
  ...
  async function runTurbo() {
    ...
    try {
      ...
      console.error(errorCaught); el("status").textContent = errorCaught.message || "Turbo Mode failed.";
    } finally { state.fighting = false; render(); }
  }
  async function replayTurbo() {
    if (!state.turboBatch) return;
    const match = matchup(); if (match.error) { el("status").textContent = match.error; return; }
    let fightNumber = null;
    try {
      fightNumber = turboView().replayNumber(state.turboBatch);
      const record = turboView().findFight(state.turboBatch, fightNumber) || state.turboBatch.errors.find((item) => item.fight_number === fightNumber);
      if (!record) throw new Error(`Fight #${fightNumber} was not found.`);
      state.fighting = true; render(); el("status").textContent = `Replaying Turbo fight #${fightNumber} from seed ${record.seed}…`;
      const replay = turbo().runSeeded(state.turboBatch.selection, record.seed), battle = replay.battle;
      await window.playIronPitBattle(battle, { heroes: match.heroes.indexes, monsters: match.monsters.indexes });
      view().writeLog(battle); view().showResult(battle); state.hasRun = true;
      el("lab-summary").textContent = `Turbo fight #${fightNumber} replay · seed ${record.seed} · ${replay.rolls.length} deterministic dice rolls.`;
      el("status").textContent = `Turbo fight #${fightNumber} replayed exactly from its recorded seed.`;
    } catch (errorCaught) {
      console.error(errorCaught); el("status").textContent = fightNumber ? `Turbo fight #${fightNumber} reproduced an engine error: ${errorCaught.message}` : errorCaught.message;
    } finally { state.fighting = false; render(); }
  }
  function resetFight() {
  ...
  function loadSample() {
    ...
    invalidateRun(); clearResult("Sample loaded: Karnok + Seraphine vs Goblin Warrior + Wolf. Press FIGHT or TURBO."); render(); el("status").textContent = "Sample loaded. Press FIGHT or TURBO.";
  }
  async function boot() {
    if (window.IRON_PIT_CANONICAL_MONSTERS_READY !== true) throw new Error("Canonical RAW-certified monster bundle did not load.");
    const required = [window.IRON_PIT_BROWSER_ENGINE, window.IRON_PIT_BROWSER_CATALOG, window.IRON_PIT_ENCOUNTER_PICKER, view(), picker(), lab(), dice(), turbo(), turboView()];
    ...
    state.catalog = await window.IRON_PIT_BROWSER_CATALOG.buildCatalog(); picker().bind(() => state); render(); el("status").textContent = "Iron Pit ready. Choose cards or load the sample matchup.";
  }
  el("fight-button").addEventListener("click", fight); el("rerun-button").addEventListener("click", fight);
  el("turbo-button").addEventListener("click", runTurbo); el("turbo-replay-button").addEventListener("click", replayTurbo);
