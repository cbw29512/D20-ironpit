(() => {
  "use strict";
  const MAX_SLOTS = 6;
  const state = { catalog: null, heroSlots: Array(MAX_SLOTS).fill(null), monsterSlots: Array(MAX_SLOTS).fill(null), fighting: false, lastScenarioKey: null, lastFingerprint: null };
  ...
  function invalidateReplay() { state.lastScenarioKey = null; state.lastFingerprint = null; updateLabControls(); }
  function render() { if (state.catalog) view().render(state, openSlot); updateLabControls(); }
  ...
  function scenarioKey(heroes, monsters, seed) {
    return JSON.stringify({ hero_ids: heroes.cards.map((card) => card.runnable_template_id), monster_ids: monsters.cards.map((card) => card.runnable_template_id), seed });
  }
  function configureDice(seed) { window.IRON_PIT_DICE = seed ? lab().createSeededDice(seed) : secureDice; }
  async function fight() {
    ...
    const seed = el("battle-seed").value.trim(), key = scenarioKey(heroes, monsters, seed);
    ...
    try {
      ...
      await new Promise((resolve) => requestAnimationFrame(resolve)); configureDice(seed);
      const battle = window.IRON_PIT_BROWSER_ENGINE.runEncounter({
        hero_ids: heroes.cards.map((card) => card.runnable_template_id), monster_ids: monsters.cards.map((card) => card.runnable_template_id),
      });
      const currentFingerprint = lab().fingerprint(battle);
      let replayStatus = "";
      if (previousFingerprint) replayStatus = previousFingerprint === currentFingerprint ? "exact replay reproduced" : "REPLAY MISMATCH — investigate engine nondeterminism";
      await window.playIronPitBattle(battle, { heroes: heroes.indexes, monsters: monsters.indexes }, { instant: el("instant-mode").checked });
    ...
    } finally { window.IRON_PIT_DICE = secureDice; state.fighting = false; render(); }
  }
  function resetFight() {
    if (state.fighting) return;
    clearResult("Battle reset. Cards and test seed are still loaded."); render();
  ...
  async function quickTest() {
    ...
    if ([...heroes, ...monsters].some((card) => !card)) { el("status").textContent = "Quick Test could not find its certified smoke-test cards."; return; }
    ...
    el("battle-seed").value = "iron-pit-smoke-1"; el("instant-mode").checked = true;
    invalidateReplay(); clearResult("Quick Test loaded: Karnok + Seraphine vs Goblin Warrior + Wolf."); render(); await fight();
  }
  async function boot() {
    if (window.IRON_PIT_CANONICAL_MONSTERS_READY !== true) throw new Error("Canonical RAW-certified monster bundle did not load.");
    const required = [window.IRON_PIT_BROWSER_ENGINE, window.IRON_PIT_BROWSER_CATALOG, window.IRON_PIT_ENCOUNTER_PICKER, view(), picker(), lab(), secureDice];
    ...
    picker().bind(() => state); render(); el("status").textContent = "Battle Lab ready. Choose cards or hit QUICK TEST.";
  }
