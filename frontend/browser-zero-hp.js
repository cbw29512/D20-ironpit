(() => {
  ...
  const DODGE = "dodge", PRONE = "prone";
  const defersDeath = (state) => (state.template.regeneration || []).some((effect) => effect.requiresPositiveHp === false);
  ...
  function useUndeadFortitude(state, incoming, damageTypes, critical) {
    ...
    const resolver = U(); if (!resolver) throw new Error("Undead Fortitude runtime is not loaded.");
  ...
  function finish(state, outcome, incoming, affectedStates) {
    ...
    C().resolveDamage(state, incoming, affectedStates); return outcome;
  }
  ...
  function applyDamage(state, amount, critical = false, damageTypes = [], affectedStates = []) {
    ...
    if (state.current_hp === 0) {
      if (state.template.kind === "monster") {
        if (defersDeath(state)) return finish(state, "damaged", incoming, affectedStates);
        markDead(state); return finish(state, "dead", incoming, affectedStates);
      }
      if (incoming >= S().effectiveMaxHp(state)) { markDead(state); return finish(state, "dead", incoming, affectedStates); }
  ...
  window.IRON_PIT_BROWSER_ZERO_HP = { applyDamage, defersDeath };
})();