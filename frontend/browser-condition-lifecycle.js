(() => {
  ...
  function hitTargetEligible(control, state) {
  ...
  function resolveHitControl(attack, attacker, target, round) {
    const control = attack.controlEffect, empty = { applied: [], saveRoll: null, saveAbility: null, saveDc: null, saveSucceeded: null };
    if (!control?.conditionId || !control.initialSaveAbility || target.state.is_dead || !target.state.is_alive || !hitTargetEligible(control, target.state)) return empty;
  ...
  window.IRON_PIT_BROWSER_HIT_CONTROL = { eligible: hitTargetEligible, resolve: resolveHitControl };
