...
const state = { template, active_modifiers: window.IRON_PIT_BROWSER_OPENING_MODIFIERS.build(template) };
const rules = window.IRON_PIT_BROWSER_DEFENSIVE_MODIFIERS;

assert.equal(state.active_modifiers.length, 3);
assert.equal(rules.attacksAgainstDisadvantage(state, { creature_type: "fiend" }), 1);
assert.equal(rules.attacksAgainstDisadvantage(state, { creature_type: "humanoid" }), 0);
assert.equal(rules.conditionImmune(state, "charmed", { creature_type: "undead" }), true);
assert.equal(rules.conditionImmune(state, "frightened", { creature_type: "fey" }), true);
assert.equal(rules.conditionImmune(state, "charmed", { creature_type: "humanoid" }), false);
assert.ok(state.active_modifiers.every((item) => item.source_effect_id === "purity-of-spirit"));
