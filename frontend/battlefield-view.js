(() => {
  ...
  function runtimeTemplate(card, side) {
    ...
    if (side === "heroes") return window.IRON_PIT_BROWSER_HEROES?.[card.runnable_template_id] || null;
    if (card.ruleset === "2014") return window.IRON_PIT_BROWSER_MONSTERS_2014?.[card.runnable_template_id] || null;
    return window.IRON_PIT_BROWSER_MONSTERS?.[card.runnable_template_id] || null;
  }
