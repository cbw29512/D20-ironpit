(() => {
  ...
  function resolveSpark(sequence, round, cleric, setup, choice) {
    ...
    const remaining = spend(cleric), count = cleric.state.template.feature_dice_counts?.[SPARK] || 1;
    const maximized = choice.kind === "divine-spark-heal" && H().healingMaximized(cleric, target, SPARK);
    const rolls = Array.from({ length: count }, () => maximized ? 8 : D().roll(8));
    const mod = wisdomModifier(cleric), total = rolls.reduce((sum, die) => sum + die, 0) + mod;
    const notation = `${count}d8+${mod}`;
