(() => {
  ...
  function occupiedByOther(target, candidate, setup) {
    return [...setup.heroes, ...setup.monsters].some((member) => {
      ...
      return G().overlaps(candidate, target.state.template.size, member.state.position, member.state.template.size);
    });
  }

  function moveRelative(source, target, distanceFt, setup, toward) {
    if (!Number.isInteger(distanceFt) || distanceFt < 0 || distanceFt % 5) {
      throw new Error("Forced movement distance must be a nonnegative 5-foot increment.");
    }
    if (!distanceFt) return 0;
    if (!setup?.map_definition || !target.state.position) {
      throw new Error("Forced movement requires an authoritative battle map and target position.");
    }
    const [sx, sy] = center(source), [tx, ty] = center(target);
    const dx = toward ? sign(sx - tx) : sign(tx - sx);
    const dy = toward ? sign(sy - ty) : sign(ty - sy);
  ...
  window.IRON_PIT_BROWSER_FORCED_MOVEMENT = { pullToward, pushAway };
})();
