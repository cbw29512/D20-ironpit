(() => {
  ...
  const G = () => window.IRON_PIT_BROWSER_GRAPPLE, T = () => window.IRON_PIT_BROWSER_TIMED, Z = () => window.IRON_PIT_BROWSER_ZERO_HP, P = () => window.IRON_PIT_BROWSER_PERSISTENT_EFFECTS, U = () => window.IRON_PIT_BROWSER_RESOURCES || { canUse: () => true, spend: () => null };
  ...
  function resolveAttack(sequence, round, attacker, target, attack, distance, extra = {}) {
    ...
    if (!U().canUse(attacker.state, attack)) throw new Error(`Resource is unavailable for attack ${attack.id}.`);
    ...
    const resourceRemaining = U().spend(attacker.state, attack); if (spendAction) E().spend(attacker.state, "action");
    ...
    if (hit) {
      ...
      const persistent = attack.persistentEffects?.length ? attack.persistentEffects : (attack.controlEffect ? [attack.controlEffect] : []);
      if (living && P()) applied.push(...P().apply(actualTarget, persistent, attacker.combatant_id, attack.id, attack.reach || 5, round));
      else if (living && attack.controlEffect) {
        const control = attack.controlEffect;
        if (control.grappleEscapeDc && (!control.maxTargetSize || S().sizeAtMost(actualTarget, control.maxTargetSize))) applied.push(...G().apply(actualTarget.state, attacker.combatant_id, control.grappleEscapeDc, attack.reach || 5, Boolean(control.restrainsWhileGrappled)));
        if (control.conditionId) { const timed = T().apply(actualTarget.state, control.conditionId, attacker.combatant_id, { sourceEffectId: attack.id, appliedRound: round, expiresAtStartOfSourceTurn: Boolean(control.expiresAtStartOfSourceTurn), expiryTiming: control.expiryTiming || null, repeatSaveAbility: control.repeatSaveAbility || null, repeatSaveDc: control.repeatSaveDc || null, repeatSaveTiming: control.repeatSaveTiming || null, allowedRemovalActionIds: control.allowedRemovalActionIds || [] }); if (timed) applied.push(timed); }
      }
    ...
    if (resourceRemaining !== null) description += ` ${attack.name} resource remaining: ${resourceRemaining}.`;
    ...
    const event = { sequence, round_number: round, event_type: "attack", actor_id: attacker.combatant_id, actor_name: attacker.state.template.name,
      ...
      resource_remaining: resourceRemaining, animation: attack.animation || (attack.kind === "ranged" ? "projectile" : "slash"), description };
