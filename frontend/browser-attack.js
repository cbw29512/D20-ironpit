(() => {
  ...
  function resolveAttack(sequence, round, attacker, target, attack, distance, extra = {}) {
    ...
    const rollContext = RC().create({ baseDisadvantageSources: disadvantage, rangedDisadvantage });
    const preRoll = H().runPhase(H().PHASES.BEFORE_ATTACK_ROLL, {
      ...
      attackRollContext: rollContext, events: [],
    });
    ...
    const advantage = (extra.advantage || 0) + conditions.advantage + RC().advantageTotal(rollContext)
