(() => {
  ...
  const STUDY = () => window.IRON_PIT_BROWSER_STUDIED_ATTACKS || { apply: () => false }, PA = () => window.IRON_PIT_BROWSER_PEERLESS_AIM || { resolve: (_state, hit) => ({ hit, used: false }) };
  ...
  const M = () => window.IRON_PIT_BROWSER_MODIFIERS || {
    attacksAgainstAdvantage: () => 0, consumeAttacksAgainstAdvantage: () => 0, nextAttackAdvantage: () => 0, consumeNextAttackAdvantage: () => 0, nextAttackAgainstAdvantage: () => 0,
  ...
  }; const MAA = (state) => M().nextAttackAdvantage?.(state) || 0, CMA = (state) => M().consumeNextAttackAdvantage?.(state) || 0, MAD = (state) => M().nextAttackDisadvantage?.(state) || 0, CMD = (state) => M().consumeNextAttackDisadvantage?.(state) || 0, C = () => window.IRON_PIT_BROWSER_CONCENTRATION, I = () => window.IRON_PIT_BROWSER_CONDITION_IMMUNITY || { immune: () => false }, F = () => window.IRON_PIT_BROWSER_FRIGHTENED;
  ...
  const E = () => window.IRON_PIT_ACTION_ECONOMY || { available: (state, cost) => cost === "action" && state.action_available, spend: (state) => { state.action_available = false; } }, RES = () => window.IRON_PIT_BROWSER_RESOURCES;
  const states = (setup) => setup ? [...setup.heroes, ...setup.monsters].map((member) => member.state) : [];
  function conditionSources(attacker, defender, distance, targetId, setup = null) {
    ...
    if (setup && Q().has(attacker, "frightened")) { if (!F()) throw new Error("Browser Frightened runtime is not loaded."); disadvantage += F().d20Disadvantage(attacker, setup); }
  ...
  function resolveAttack(sequence, round, attacker, target, attack, distance, extra = {}) {
    ...
    const advantage = (extra.advantage || 0) + conditions.advantage + R().bloodiedAttackAdvantage(attacker.state, attack)
      + B2().attackAdvantage(attacker.state, attack) + A().sources(attack, target.state, attacker.combatant_id) + MAA(attacker.state) + M().nextAttackAgainstAdvantage(attacker.state, target.combatant_id);
    ...
    M().consumeNextAttackAgainstAdvantage(attacker.state, target.combatant_id); CMA(attacker.state); CMD(attacker.state); SAP().consume(attacker.state);
    ...
    const aimed = PA().resolve(attacker.state, parry.hit), hit = aimed.hit, targetAc = baseTargetAc + (parry.used ? actualTarget.state.template.parry_reaction.ac_bonus : 0);
    ...
    if (heroic.used) description += " Heroic Inspiration rerolls one d20."; if (aimed.used) description += " Peerless Aim converts the miss into a hit.";
    ...
    const event = { sequence, round_number: round, event_type: "attack", actor_id: attacker.combatant_id, actor_name: attacker.state.template.name,
      ...
      feature_id: extra.featureId || (recklessStarted ? "reckless-attack" : null), resource_remaining: resourceRemaining, concentration_ended_effect_id: concentrationBefore && !actualTarget.state.concentration ? concentrationBefore : null,
