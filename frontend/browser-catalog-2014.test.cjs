...
(async () => {
  ...
  assert.equal(catalog.hero_ready_count, 77);
  assert.equal(catalog.monster_count, 327);
  assert.equal(catalog.monster_ready_count, 128);
  assert.equal(catalog.heroes.length, 77);
  assert.equal(catalog.monsters.length, 128);
  ...
  assert.deepEqual(rogues.map((card) => card.level), Array.from({ length: 16 }, (_, i) => i + 1));
  ...
  console.log("2014 catalog exposes Fighter 1-20, Berserker 1-20, Thief 1-16, Open Hand Monk 1-10, and Devotion Paladin 1-11.");
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});