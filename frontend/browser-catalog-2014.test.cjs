...
(async () => {
  ...
  assert.deepEqual(barbarians.map((card) => card.level), Array.from({ length: 13 }, (_, i) => i + 1));
  ...
  console.log("2014 catalog exposes Fighter 1-20, Berserker 1-13, and Thief, Open Hand Monk, Devotion Paladin 1-10.");
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});