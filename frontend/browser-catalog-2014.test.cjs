...
(async () => {
  ...
  assert.deepEqual(paladins.map((card) => card.level), Array.from({ length: 14 }, (_, i) => i + 1));
  ...
  console.log("2014 catalog exposes Fighter 1-20, Berserker 1-20, Thief 1-20, Open Hand Monk 1-20, and Devotion Paladin 1-13.");
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});