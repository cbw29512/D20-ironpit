...
(async () => {
  ...
  assert.deepEqual(monks.map((card) => card.level), Array.from({ length: 16 }, (_, i) => i + 1));
  ...
  console.log("2014 catalog exposes Fighter 1-20, Berserker 1-20, Thief 1-20, Open Hand Monk 1-16, and Devotion Paladin 1-12.");
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});