...
(async () => {
  ...
  assert.deepEqual(rogues.map((card) => card.level), Array.from({ length: 11 }, (_, i) => i + 1));
  ...
  console.log("2014 catalog exposes Fighter 1-20, Berserker 1-20, Thief 1-11, Open Hand Monk 1-10, and Devotion Paladin 1-11.");
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});