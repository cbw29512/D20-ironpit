...
The browser production `beforeAttackRoll` phase now owns the named pre-roll Advantage participants that were previously sequenced directly inside `browser-attack.js`. The preserved order is `Reckless Attack -> Brutal Strike suppression -> Bloodied Fury`. A typed mutable attack-roll context records independent Advantage sources, pre-existing Disadvantage facts, aggregate description fragments, and the optional feature id without emitting sibling battle events or advancing sequence. Reckless Attack participates when either the attacker can use Reckless Attack or the defender is already reckless, so generic attackers retain Advantage against a reckless defender. Brutal Strike suppresses only the Reckless Attack Advantage source; Bloodied Fury remains an independent source.

This migration deliberately does not move Heroic Inspiration, which occurs only after an initial failed d20 roll, nor Rage maintenance, Redirect Attack, Parry, or post-attack Champion movement. Those are different timing windows/shared lifecycle mechanics and must not be mislabeled as `beforeAttackRoll` hooks merely to remove names from the attack file.
